...
public interface Statement
{
   ...
   * closed, its current <code>ResultSet</code> object, if one exists, is also
   * closed.
   * 
   ...
   * <code>LONGVARBINARY</code>,<code>CHAR</code>,<code>VARCHAR</code>,
   * and <code>LONGVARCHAR</code> fields. If the limit is exceeded, the excess
   ...
   * 
   * @param seconds the new query timeout limit in seconds; zero means there is
   *          no limit
   * @exception SQLException if a database access error occurs or the condition
   ...
   * in <code>ResultSet</code> objects created using this
   ...
   * default for result sets generated from this <code>Statement</code>
   ...
   * @exception SQLException if a database access error occurs, or the condition
   *              0 <=<code>rows</code><=<code>
   *              this.getMaxRows()</code>
   *              is not satisfied.
   * @since 1.2
   * @see #getFetchSize
   */
   ...
   * returned by the method <code>executeBatch</code> may be one of the
   * following:
   * <OL>
   * <LI>A number greater than or equal to zero -- indicates that the command
   * was processed successfully and is an update count giving the number of rows
   * in the database that were affected by the command's execution
   * <LI>A value of <code>SUCCESS_NO_INFO</code>-- indicates that the
   * command was processed successfully but that the number of rows affected is
   * unknown
   * <P>
   ...
   * returned by the method <code>BatchUpdateException.getUpdateCounts</code>
   * will contain as many elements as there are commands in the batch, and at
