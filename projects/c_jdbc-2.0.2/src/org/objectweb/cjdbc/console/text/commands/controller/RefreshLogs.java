...
public class RefreshLogs extends ConsoleCommand
{
  ...
  public void parse(String commandText) throws IOException, ConsoleException
  {
    try
    {
      ...
      console.printInfo(ConsoleTranslate
          .get("controller.command.refresh.logs.completed"));
    }
    catch (Exception e)
    {
      throw new ConsoleException(ConsoleTranslate.get(
          "controller.command.refresh.logs.failed", e));
    }
  ...
  {
    return "reload logging configuration";
  }
  ...
  {
    return ConsoleTranslate.get("controller.command.refresh.logs.description");
  }
}
