...
public class ViewCheckpointNames extends AbstractAdminCommand
{
  ...
  {
    console.println(ConsoleTranslate.get("admin.command.view.checkpoints.echo",
        dbName));
    VirtualDatabaseMBean vdjc = jmxClient.getVirtualDatabaseProxy(dbName, user,
        password);
    ArrayList list = vdjc.viewCheckpointNames();
    for (int i = 0; i < list.size(); i++)
      console.println("[" + i + "]\t" + list.get(i));
  }
  ...
  {
    return ConsoleTranslate.get("admin.command.view.checkpoints.description");
  }
