 ...
 */
public class ShowLoggingConfig extends ConsoleCommand
{

  /**
   * Creates a new "show logging config" controller command.
   * 
   * @param module the command is attached to
   */
  public ShowLoggingConfig(AbstractConsoleModule module)
  {
    super(module);
  }
  ...
  {
    if (commandText == null || commandText.equalsIgnoreCase(""))
    {
      // Display on std output
      console.printInfo(jmxClient.getControllerProxy().getLoggingConfiguration());
    }
    else
    {
      // Write to file
      File f = new File(commandText.trim());
      BufferedWriter writer = new BufferedWriter(new FileWriter(f));
      writer.write(jmxClient.getControllerProxy().generateReport());
      writer.flush();
      writer.close();
    }
  }
  ...
  {
    return ConsoleTranslate.get("controller.command.show.logging.config");
  }
