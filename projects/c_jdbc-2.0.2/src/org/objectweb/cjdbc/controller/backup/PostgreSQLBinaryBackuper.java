...
/**
 * This class defines a Backuper for PostgreSQL databases.  This backuper 
 * creates dumps in a binary formatu, using the "--format=c" switch on pg_dump.
 * <p>
 ...
 * makes calls to the pg_dump, createdb, dropdb and pg_restore commands.
 * 
 * @author <a href="mailto:dhansen@h2st.com">Dylan Hansen</a>
 * @version 1.1
 */
public class PostgreSQLBinaryBackuper extends AbstractPostgreSQLBackuper
{
  // Logger
  ...
  /**
   * Creates a new <code>PostgreSQLBinaryBackuper</code> object
   * 
   * @author Dylan Hansen
   */
      ...
      String dumpName, String path, ArrayList tables) throws BackupException
  {
    ...
    try
    {
      ...
      String[] cmd = {"pg_dump", "--format=c", "-f", fullPath, "-h", host, "-p", port, "-U",
          login, dbName};
      ...
      String dumpName, String path, ArrayList tables) throws BackupException
  {
    ...
    try
    {
      ...
      String[] replayCmd = {"pg_restore", "-h", host, "-p", port, "-U", login, "--format=c", "-d", dbName, fullPath};
