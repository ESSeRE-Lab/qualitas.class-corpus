 ...
 * @version 1.0
 */
public class RollbackToSavepoint extends DistributedTransactionMarker
{
   ...
   */
  public RollbackToSavepoint(long transactionId, String savepointName)
  {
    super(transactionId);
      ...
      throws SQLException
  {
    try
    {
      ...
      {
        drm.getRecoveryLog().logRollback(tm, savepointName);
      }
    }
        ...
        if (drm.getLogger().isDebugEnabled())
          drm.getLogger().debug(
              Translate.get(
                  ...
                  new String[]{savepointName, String.valueOf(transactionId)}));
        long logId = drm.getRecoveryLog().logRollback(tm, savepointName);
    ...
    {
      AbstractRequest request = new UnknownRequest("rollback " + savepointName,
          false, 0, "\n");
  ...
  {
    if (super.equals(obj))
      return savepointName.equals(((RollbackToSavepoint) obj)
          .getSavepointName());
    else
      return false;
  }
