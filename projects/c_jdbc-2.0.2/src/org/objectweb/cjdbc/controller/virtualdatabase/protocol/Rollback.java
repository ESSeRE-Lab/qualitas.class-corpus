 ...
 * @version 1.0
 */
public class Rollback extends DistributedTransactionMarker
{
   ...
   */
  public Rollback(String login, long transactionId)
  {
    super(transactionId);
    this.login = login;
  }
      ...
      throws SQLException
  {
    ...
    {
      drm.getLogger().warn(
          Translate.get("virtualdatabase.distributed.rollback.exception"), re);
      ...
      throws SQLException
  {
    ...
    try
    {
      ...
      if (logger.isDebugEnabled())
        logger.debug(Translate.get("transaction.rollback", "" + tid));
      ...
      if (drm.getRecoveryLog() != null)
        drm.getRecoveryLog().logRollback(tm);
    }
      ...
      if (drm.getRecoveryLog() != null)
      {
        ...
        { // Wait to be sure that we log in the proper order
          Rollback totalOrderRollback = new Rollback(tm.getLogin(),
              transactionId);
          if (drm.getLoadBalancer()
              .waitForTotalOrder(totalOrderRollback, false))
            drm.getLoadBalancer().removeHeadFromAndNotifyTotalOrderQueue();
        }
        long logId = drm.getRecoveryLog().logRollback(tm);
