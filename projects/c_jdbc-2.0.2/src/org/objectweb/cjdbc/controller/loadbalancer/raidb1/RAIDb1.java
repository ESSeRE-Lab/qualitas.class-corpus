...
public abstract class RAIDb1 extends AbstractLoadBalancer
{
  ...
  /**
   * List of <code>BackendWorkerThread</code> that executes possibly blocking
   * queries
   */
  protected ArrayList                   backendBlockingThreads;
  /**
   * List of <code>BackendWorkerThread</code> that executes non-blocking
   * queries
   */
  ...
  {
    super(vdb, RAIDbLevels.RAIDb1, ParsingGranularities.NO_PARSING);
   ...
   * @exception NoMoreBackendException if no backends left to execute the
   *              request
   */
      ...
      throws AllBackendsFailedException, NoMoreBackendException, SQLException
  {
    ...
    { // Non-blocking
      ...
      // else all previous requests for this transaction have already completed
    ...
    // Note that the backendThreads list is only supposed to contain enabled
