...
public class WebSessionCtx implements Serializable
{
	...
	/** SSL Context					*/
	public final static String		CTX_SERVER_CONTEXT_SSL = "contextSSL";
	/** Plain Context				*/
	public final static String		CTX_SERVER_CONTEXT_PLAIN = "contextPlain";
	/** Document Directory	*/
	...
	private WebSessionCtx (HttpServletRequest request)
	{
		...
		if (request.isSecure())
		{
			String serverContext = "https://" + request.getServerName()
				+ ":" + request.getServerPort() 
				+ request.getContextPath();
			ctx.put(CTX_SERVER_CONTEXT_SSL, serverContext);
			ctx.put(CTX_SERVER_CONTEXT, serverContext);
			ctx.put(CTX_SERVER_CONTEXT_PLAIN, "http://" + request.getServerName() 
				+ request.getContextPath());
		}
		else
		{
			String serverContext = "http://" + request.getServerName()
				+ ":" + request.getServerPort() 
				+ request.getContextPath();
			ctx.put(CTX_SERVER_CONTEXT_PLAIN, serverContext);
			ctx.put(CTX_SERVER_CONTEXT, serverContext);
			ctx.put(CTX_SERVER_CONTEXT_SSL, "https://" + request.getServerName() 
			+ request.getContextPath());
		}
