...
@Rem  $Id: CLInstall.bat 7224 2009-02-20 20:13:19Z cjandhyala $
@Echo off
...
set migrate=
@IF "%2"=="migrate" set migrate=--migrate y
...
@Rem Pause
@"%JAVA%" -Xmx258m -classpath %CP% -DCOMPIERE_HOME=%CL_COMPIERE_HOME% com.compiere.client.CommandLineInstall --email %CL_WEBSTORE_EMAIL% --sysname %CL_SYSTEM_NAME% --pwd %CL_WEBSTORE_PASS% %migrate%
@Echo ErrorLevel = %ERRORLEVEL%
