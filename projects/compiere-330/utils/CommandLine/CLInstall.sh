...
# $Id: CLInstall.sh 7087 2009-01-23 00:31:17Z freyes $

if !(command -v sqlplus > /dev/null) then
    echo sqlplus not found
    echo Please ensure that the sqlplus command can be found in the PATH.
    exit 1;
fi

if !(command -v curl > /dev/null) then
    echo curl not found
    echo Please ensure that the curl command can be found in the PATH.
    exit 1;
fi
...
echo $JAVA -Xmx258m -classpath $CP -DCOMPIERE_HOME=$CL_COMPIERE_HOME com.compiere.client.CommandLineInstall --email $CL_WEBSTORE_EMAIL --sysname $CL_SYSTEM_NAME --pwd $CL_WEBSTORE_PASS

$JAVA -Xmx258m -classpath $CP -DCOMPIERE_HOME=$CL_COMPIERE_HOME com.compiere.client.CommandLineInstall --email $CL_WEBSTORE_EMAIL --sysname $CL_SYSTEM_NAME --pwd $CL_WEBSTORE_PASS $2
