...
{	
	...
	protected int getC_Period_ID()
	{
		if (m_C_Period_ID == -1)
		{
			...
			MPeriod period = MPeriod.getOfOrg(p_po.getCtx(), getAD_Org_ID(), getDateAcct());
			if (period != null)
				m_C_Period_ID = period.getC_Period_ID();
			if (m_C_Period_ID == -1)
				m_C_Period_ID = 0;
		}
	...
	public boolean isItem()
	{
		...
		{
			MProduct product = MProduct.get(p_po.getCtx(), getM_Product_ID());
	...
	public ProductCost getProductCost()
	{
		if (m_productCost == null)
			m_productCost = new ProductCost (p_po.getCtx(), 
				getM_Product_ID(), getM_AttributeSetInstance_ID(), p_po.get_Trx());
	 ...
	 * 	@param isSOTrx SL order p_trx (i.e. negative qty)
	 */
