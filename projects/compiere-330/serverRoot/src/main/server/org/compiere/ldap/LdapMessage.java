...
public class LdapMessage
{
	...
	public void decode(byte[] data, int length)
	{
		...
		{
			StringBuffer info = new StringBuffer("#").append(data.length).append(": ");
			for (int i = 0; i < data.length; i++)
            {
	            ...
	            if (Character.isDefined(c))
	            	...
	            	info.append("[").append(data[i]) .append("]");
	        //	Arrays.toString(data);
            }
			log.log(Level.WARNING, info.toString(), e);
			return;
		}
		...
		{
			result.setErrorNo(LdapResult.LDAP_PROTOCOL_ERROR);
			StringBuffer info = new StringBuffer("Protocol Error: #").append(data.length).append(": ");
			for (int i = 0; i < data.length; i++)
            {
	            char c = (char)data[i];
	            if (Character.isDefined(c))
			...
			log.log(Level.WARNING, info.toString(), ex);
		}
	}	//	decode
	...
	{
		log.config("#" + msgId + ": SEARCH_REQUEST");
		...
		while (true)
		{
			...
			if (filter == FILTER_EQUALITYMATCH)
			{
				...
				log.config("#" + msgId + ": userId = " + userId);
				equalityFilter = true;
				break;
			}
			else if (filter == FILTER_AND)
			{
				...
				log.fine("#" + msgId + ": got FILTER_AND");
			}
			else if (filter == SEQUENCE)
			{
				log.fine("#" + msgId + ": got SEQUENCE");
				break;
			}
		}  // while true
	 ...
	 */
	private void handleBind() throws Exception
	{
		...
		// Log the information 
		log.config("#" + msgId + ": bind - version=" + version + ", userId=" + userId);
