 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: CompiereMonitor.java,v 1.3 2006/07/30 00:53:33 jjanke Exp $
 */
public class CompiereMonitor extends HttpServlet
{
		...
		throws ServletException, IOException
	{
		...
		{
			CompiereProcessorLog pLog = logs[i];
			line = new tr();
		...
		throws ServletException, IOException
	{
		...
		MClient client = MClient.get(new Ctx(), AD_Client_ID);
		...
		throws ServletException, IOException
	{
		...
		if (cmd == null || cmd.length() == 0) {
			cmd = WebUtil.getParameter (request, "RunGC");
		...
		throws ServletException, IOException
	{
		...
		System.err.println("CompiereMonitor: " + CLogMgt.class.toString() + " @" + Integer.toHexString(CLogMgt.class.hashCode()));
		System.err.println("CompiereMonitor: " + m_serverMgr.toString());
		 **/
		...
		bb.addElement(new hr());
		para = new p();
		ArrayList<CompiereServer> servers = m_serverMgr.getAll();		
		for (int i = 0; i < servers.size(); i++)
		{
			if (i > 0)
				para.addElement(new br());
			CompiereServer server = servers.get(i);
		...
		//	***** Server Details *****
		for (CompiereServer server : servers)
		{
			bb.addElement(new hr());
	...
	private void createLogMgtPage (body bb)
	{
		...
		tr line = new tr();
		line.addElement(new th().addElement(system.getDBAddress(true)));
...
//		line.addElement(new td().addElement(system.getDBInstance()));
		...
		line.addElement(new th().addElement("NonHeap Memory"));
		...
		td t = new td();
		t.addElement(new a("compiereMonitor?RunGC=Yes", "Run Garbage Collect"));
		t.addElement(new br());
		t.addElement(new a("compiereMonitor?CacheReset=Yes", "Reset Cache"));
		line.addElement(t);
		table.addElement(line);
		...
		if (logDir != null && logDir.isDirectory())
		{
			...
			for (File log : logs) 
			{
				...
				int index = fileNameDisplay.lastIndexOf("/");
				if (index == -1)
					index = fileNameDisplay.lastIndexOf("\\");
				if (index != -1)
					fileNameDisplay = fileNameDisplay.substring(index+1);
				a link = new a ("compiereMonitor?Trace=" + fileName, fileNameDisplay);
				p.addElement(link);
			...
			p.addElement("FileHandler=" + fileHandler);
		bb.addElement(p);
