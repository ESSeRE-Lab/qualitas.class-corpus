...
WHERE	UPPER(ColumnName) IN ('CREATEDBY', 'UPDATEDBY') 
	AND (AD_Reference_ID != 18 OR AD_Reference_Value_ID != 110);
...
--	set Updated, Created to not updateable
SELECT '<<< set Client/Org Trx Security >>>' FROM DUAL;

UPDATE AD_Column
SET AD_Val_Rule_ID=116 -- Client Login
WHERE ColumnName='AD_Client_ID' 
AND (AD_Val_Rule_ID IS NULL OR AD_Val_Rule_ID <> 116)
AND AD_Table_ID IN (SELECT AD_Table_ID FROM AD_Table WHERE AccessLevel=4);

UPDATE AD_Column
SET AD_Val_Rule_ID=129 -- Client Trx Security
WHERE ColumnName='AD_Client_ID' 
AND (AD_Val_Rule_ID IS NULL OR AD_Val_Rule_ID <> 129)
AND AD_Table_ID IN (SELECT AD_Table_ID FROM AD_Table WHERE AccessLevel IN (1,3));

UPDATE AD_Column
SET AD_Val_Rule_ID=130  -- Org Trx Security
WHERE ColumnName='AD_Org_ID' 
AND (AD_Val_Rule_ID IS NULL OR AD_Val_Rule_ID <> 130)
AND AD_Table_ID in (SELECT AD_Table_ID FROM AD_Table WHERE AccessLevel=1);
