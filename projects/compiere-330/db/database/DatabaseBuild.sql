...
-- CREATED BY/DATE     : Rapid SQL on 10/17/2007 11:10:15.320
...
BEGIN
	...
	BEGIN
		...
		SELECT	MAX(C_Currency_ID),MAX(C_ConversionType_ID),SUM(GrandTotal), MAX(MultiplierAP), MAX(Multiplier)
		  INTO	v_Currency_ID, v_CurrencyType_ID, v_TotalOpenAmt, v_MultiplierAP, v_MultiplierCM
		FROM	C_Invoice_v		--	corrected for CM / Split Payment
		WHERE	C_Invoice_ID = p_C_Invoice_ID
                AND     NVL(C_InvoicePaySchedule_ID,0) = NVL(p_C_InvoicePaySchedule_ID,0);
          ELSE
                SELECT	MAX(C_Currency_ID),MAX(C_ConversionType_ID),SUM(GrandTotal), MAX(MultiplierAP), MAX(Multiplier)
		  INTO	v_Currency_ID, v_CurrencyType_ID, v_TotalOpenAmt, v_MultiplierAP, v_MultiplierCM
		FROM	C_Invoice_v		--	corrected for CM / Split Payment
	...
	FOR a IN Cur_Alloc LOOP
        ...
        -- Allocation
			+ currencyConvert(v_Temp * v_MultiplierAP,
				a.C_Currency_ID, v_Currency_ID, a.DateTrx, v_CurrencyType_ID, a.AD_Client_ID, a.AD_Org_ID);
    ...
    IF (p_C_InvoicePaySchedule_ID > 0) THEN --   if not valid = lists invoice amount
        ...
        FOR s IN Cur_PaySchedule LOOP
            IF (s.C_InvoicePaySchedule_ID = p_C_InvoicePaySchedule_ID) THEN
              IF (s.DueAmt > 0) THEN-- positive
                IF (s.DueAmt < v_Remaining) THEN-- paid more
                  v_TotalOpenAmt := 0;
                ELSE
                  v_TotalOpenAmt := (s.DueAmt*v_MultiplierCM) - v_Remaining;
                END IF;
              ELSE 
                IF (s.DueAmt > v_Remaining) THEN
                    v_TotalOpenAmt := 0;
                ELSE
                    v_TotalOpenAmt := (s.DueAmt*v_MultiplierCM) + v_Remaining;
                END IF;
  ...
  DBMS_OUTPUT.PUT_LINE('== Total=' || v_TotalOpenAmt);
    ...
    -- Rounding
    v_TotalOpenAmt := currencyRound(v_TotalOpenAmt, v_Currency_ID, NULL);
    
    --	Ignore Penny if there is a payment
    IF (v_paidAmt != 0 AND v_TotalOpenAmt BETWEEN -0.00999 AND 0.00999) THEN
        v_TotalOpenAmt := 0;
    END IF;
...
END currencyRound;
/
-- File c_order_drilldown.sql was not found.
...
-- File m_pricelist_create.sql was not found.
...
AS
	...
	--
        /* for constraints, joining to user_tables to prevent picking up constraints that 
           have already been dropped but are still in the recycle bin */
	CURSOR	Cur_Invalids IS
		SELECT	object_id, object_name, object_type, null table_name
		FROM	user_objects
		WHERE	status <> 'VALID'
		  AND	object_type IN ('VIEW', 'PACKAGE', 'PACKAGE BODY', 'FUNCTION', 
								'PROCEDURE', 'TRIGGER', 'JAVA CLASS', 'SYNONYM')
                union all 
                select null object_id, C.constraint_name object_name, 'CONSTRAINT' object_type, C.table_name
                from    user_constraints C, user_tables T
                where   C.table_name = T.table_name and 
                        ( C.validated <> 'VALIDATED' or
                          C.status <> 'ENABLED' )
	...
	LOOP
		...
		FOR ci IN Cur_Invalids LOOP
			...
			IF NOT invalid_tab_rec.EXISTS(ci.object_id) THEN
                                if( ci.object_type = 'CONSTRAINT' ) then
                                        sql_statement := 'alter table ' || ci.table_name || ' enable validate constraint ' || ci.object_name;
				ELSIF (ci.object_type = 'JAVA CLASS') THEN
				...
				IF Cur_Valids%ROWCOUNT > 0  or ci.object_type = 'CONSTRAINT' THEN
 ...
 * $Source: /cvs/compiere/db/database/Procedures/DBA_AfterImport.sql,v $
 ***
...
WHERE c.ColumnName <> e.ColumnName
    */
	...
	WHERE f.IsCentrallyMaintained='Y' AND f.IsActive='Y'
	  ...
	  AND EXISTS (SELECT * FROM AD_Tab t, AD_Window_v w
	...
	WHERE EXISTS (SELECT * FROM AD_Field f, AD_Element_trl e, AD_Column c
				WHERE trl.AD_Field_ID=f.AD_Field_ID
				  ...
				  AND e.PO_Name IS NOT NULL)
	  AND EXISTS (SELECT * FROM AD_Field f, AD_Tab t, AD_Window_v w
				WHERE trl.AD_Field_ID=f.AD_Field_ID
...
-- File ad_column_sync.sql was not found.
 ...
 * Test
 *		SELECT currencyRate(116, 100, null, 114,11,0) FROM DUAL;     => .647169
 *		SELECT currencyRate(100, 116, null, 114,11,0) FROM DUAL;     => NULL
 * 	SELECT currencyRate(99, 116, null, 114,11,0) FROM DUAL;     => NULL
 *     SELECT currencyRate(100, 99, null, 114,11,0) FROM DUAL;     => NULL
 ************************************************************************/
AS
	--	Currency From variables
	cf_IsEuro			CHAR(1) := NULL;
...
BEGIN
	...
	--	Get Currency Info
    BEGIN
	    SELECT	IsEuro, IsEMUMember, EMUEntryDate, EMURate
	...
	IF (cf_IsEuro IS NULL) THEN
		DBMS_OUTPUT.PUT_LINE('From Currency Not Found: ' || p_CurFrom_ID);
		RETURN NULL;
	END IF;
    BEGIN
    	SELECT	IsEuro, IsEMUMember, EMUEntryDate, EMURate
	...
	IF (ct_IsEuro IS NULL) THEN
		DBMS_OUTPUT.PUT_LINE('To Currency Not Found: ' || p_CurTo_ID);
		RETURN NULL;
	END IF;
	...
	DECLARE
		CURSOR	CUR_Rate	IS
			...
			WHERE	C_Currency_ID=v_CurrencyFrom AND C_Currency_To_ID=v_CurrencyTo
...
AS
SELECT t.AD_Window_ID, f.AD_Tab_ID, f.AD_Field_ID, 
    tbl.AD_Table_ID, f.AD_Column_ID, 
    uw.AD_Role_ID AS UserDef_Role_ID, uw.AD_User_ID,
    uw.AD_UserDef_Win_ID, uw.CustomizationName, u.AD_UserDef_Tab_ID, u.AD_UserDef_Field_ID,
    COALESCE(u.Name,f.Name) AS Name, 
    COALESCE(u.Description,f.Description) AS Description, 
    COALESCE(u.Help,f.Help) AS Help,
    ...
    f.IsHeading, f.IsFieldOnly, f.IsReadOnly, 
	f.IsEncrypted AS IsEncryptedField, f.ObscureType, f.IsDefaultFocus, 
    ...
    c.IsKey, c.IsParent,
	c.IsMandatory, c.MandatoryLogic,
	COALESCE(COALESCE(u.IsMandatoryUI,f.isMandatoryUI),c.IsMandatoryUI) AS IsMandatoryUI,    c.IsIdentifier, c.IsTranslated, c.AD_Reference_Value_ID, 
	c.Callout, c.IsCallout,
...
FROM AD_Field f 
  ...
  LEFT OUTER JOIN AD_UserDef_Field u ON (f.AD_Field_ID=u.AD_Field_ID)
  LEFT OUTER JOIN AD_UserDef_Tab ut ON (ut.AD_UserDef_Tab_ID=u.AD_UserDef_Tab_ID)
  LEFT OUTER JOIN AD_UserDef_Win uw ON (uw.AD_UserDef_Win_ID=ut.AD_UserDef_Win_ID)
WHERE f.IsActive = 'Y' 
  AND c.IsActive = 'Y'
/
...
AS
SELECT trl.AD_Language, t.AD_Window_ID, f.AD_Tab_ID, f.AD_Field_ID, 
    tbl.AD_Table_ID, f.AD_Column_ID, 
    uw.AD_Role_ID AS UserDef_Role_ID, uw.AD_User_ID,
    uw.AD_UserDef_Win_ID, uw.CustomizationName, u.AD_UserDef_Tab_ID, u.AD_UserDef_Field_ID,
...
AS
SELECT t.AD_Tab_ID, t.AD_Window_ID, t.AD_Table_ID, t.AD_CtxArea_ID,
    uw.AD_Role_ID AS UserDef_Role_ID, uw.AD_User_ID,
    uw.AD_UserDef_Win_ID, uw.CustomizationName, u.AD_UserDef_Tab_ID,
    ...
    t.AD_Column_ID, c.ColumnName AS LinkColumnName, t.AD_Process_ID, 
    t.IsSortTab, t.IsAdvancedTab,
...
FROM AD_Tab t 
  ...
  LEFT OUTER JOIN AD_Column c ON (t.AD_Column_ID=c.AD_Column_ID)
  LEFT OUTER JOIN AD_UserDef_Tab u ON (u.AD_Tab_ID=t.AD_Tab_ID)
  LEFT OUTER JOIN AD_UserDef_Win uw ON (uw.AD_UserDef_Win_ID=u.AD_UserDef_Win_ID)
WHERE t.IsActive='Y'
  AND tbl.IsActive='Y'
/
...
AS
SELECT trl.AD_Language, t.AD_Tab_ID, t.AD_Window_ID, t.AD_Table_ID, t.AD_CtxArea_ID,
    uw.AD_Role_ID AS UserDef_Role_ID, uw.AD_User_ID, 
    uw.AD_UserDef_Win_ID, uw.CustomizationName, u.AD_UserDef_Tab_ID,
    COALESCE(u.Name,trl.Name) AS Name, 
    COALESCE(u.Description,trl.Description) AS Description, 
...
-- VIEW: AD_Window_v
--
CREATE OR REPLACE VIEW AD_Window_v
AS
SELECT w.AD_Window_ID, u.AD_Client_ID,
    u.AD_Role_ID AS UserDef_Role_ID, u.AD_User_ID, u.AD_UserDef_Win_ID, u.CustomizationName,
    COALESCE(u.Name,w.Name) AS Name, 
    COALESCE(u.Description,w.Description) AS Description, 
    COALESCE(u.Help,w.Help) AS Help,
    w.WindowType, w.AD_Color_ID, w.AD_Image_ID, 
    COALESCE(u.WinHeight,w.WinHeight) AS WinHeight, 
    COALESCE(u.WinWidth,w.WinWidth) AS WinWidth, 
    w.AD_CtxArea_ID, 
    COALESCE((SELECT IsSOTrx FROM AD_CtxArea ctx WHERE w.AD_CtxArea_ID=ctx.AD_CtxArea_ID), 'Y') AS IsSOTrx, 
    a.AD_Role_ID, a.IsReadWrite
FROM AD_Window w
  INNER JOIN AD_Window_Access a ON (w.AD_Window_ID=a.AD_Window_ID)
  LEFT OUTER JOIN AD_UserDef_Win u ON (w.AD_Window_ID=u.AD_Window_ID)
WHERE w.IsActive='Y'
  AND a.IsActive='Y'
/
...
AS 
SELECT trl.AD_Language, w.AD_Window_ID,  u.AD_Client_ID,
    u.AD_Role_ID AS UserDef_Role_ID, u.AD_User_ID, u.AD_UserDef_Win_ID, u.CustomizationName,
    COALESCE(u.Name,trl.Name) AS Name, 
    COALESCE(u.Description,trl.Description) AS Description, 
    COALESCE(u.Help,trl.Help) AS Help,
    ...
    a.AD_Role_ID, a.IsReadWrite
FROM AD_Window w
  INNER JOIN AD_Window_Trl trl ON (w.AD_Window_ID=trl.AD_Window_ID)
  INNER JOIN AD_Window_Access a ON (w.AD_Window_ID=a.AD_Window_ID)
  LEFT OUTER JOIN AD_UserDef_Win u ON (w.AD_Window_ID=u.AD_Window_ID)
WHERE w.IsActive='Y'
  AND a.IsActive='Y'
...
SELECT io.AD_Client_ID, io.AD_Org_ID, io.IsActive, io.Created, io.CreatedBy, io.Updated, io.UpdatedBy,
	'en_US' AS AD_Language,
	io.M_InOut_ID, io.IsSOTrx, io.DocumentNo, io.DocStatus,	 io.C_DocType_ID, io.SalesRep_ID,
...
--  Project Line Report
CREATE OR REPLACE VIEW RV_Project_Details
AS
SELECT l.AD_Client_ID,l.AD_Org_ID,l.IsActive,l.Created,l.CreatedBy,l.Updated,l.UpdatedBy,
l.C_ProjectLine_ID, l.C_ProjectPhase_ID, l.C_ProjectTask_ID, l.C_Project_ID,
-- Line
l.Line, M_Product_ID, M_Product_Category_ID,
l.PlannedAmt, l.PlannedPrice, l.PlannedQty, l.PlannedMarginAmt,
l.Discount, l.PriceList, 
COALESCE(l.Probability, p.Probability) AS Probability,
COALESCE(l.PlannedDate, p.PlannedDate) AS PlannedDate,
COALESCE(l.Probability, p.Probability) * l.PlannedAmt AS PlannedProbability,
l.CommittedAmt, l.CommittedQty,
l.Description,
--  Project
p.Value, p.Name,
p.C_BPartnerSR_ID,p.SalesRep_ID,
p.C_BPartner_ID, p.C_BPartner_Location_ID, p.AD_User_ID,
p.C_Campaign_ID, p.C_SalesRegion_ID,
p.M_PriceList_Version_ID, p.M_Warehouse_ID,
p.C_Currency_ID, p.C_PaymentTerm_ID, 
p.C_Phase_ID, p.C_ProjectType_ID,
p.DateContract, p.DateFinish,
p.ProjInvoiceRule, p.ProjectCategory, p.ProjectLineLevel,
--  Line Reference
l.C_Order_ID, l.InvoicedAmt, l.InvoicedQty,
l.C_OrderPO_ID, l.C_ProjectIssue_ID,
l.IsPrinted, l.Processed
FROM C_ProjectLine l
INNER JOIN C_Project p ON (l.C_Project_ID=p.C_Project_ID)
/
...
AS SELECT 
	il.AD_Client_ID, il.AD_Org_ID, il.IsActive, il.Created, il.CreatedBy, il.Updated, il.UpdatedBy,        
        i.C_Invoice_ID, i.C_Order_ID, i.SalesRep_ID,
	i.C_BPartner_ID, i.C_BP_Group_ID, i.DocumentNo,
        i.DateInvoiced, i.DateAcct, i.IsSOTrx, i.C_DocType_ID, i.DocStatus, i.IsPaid,
        i.C_Currency_ID, i.C_BPartner_Location_ID, i.AD_User_ID, 
        ROUND(i.Multiplier*i.GrandTotal, 2) AS GrandTotal,
        ROUND(i.Multiplier*TotalLines, 2) AS TotalLines,
        --  User/BP
        u.EMail, u.Title, u.Phone, u.Fax, 
        --  Location
        l.Address1, l.Address2, l.City, l.Postal, l.C_Region_ID, l.C_Country_ID,         
        -- Product
	il.M_Product_ID, p.M_Product_Category_ID,
	-- Line
	il.C_InvoiceLine_ID, il.C_OrderLine_ID,
        il.C_Campaign_ID, il.C_Project_ID, il.C_Activity_ID, il.C_ProjectPhase_ID, il.C_ProjectTask_ID,
        il.C_Tax_ID, il.C_UOM_ID,
	-- Qty
...
FROM  RV_C_Invoice i
  ...
  INNER JOIN AD_User u ON (i.AD_User_ID = u.AD_User_ID) 
/
...
SELECT bp.AD_Client_ID, bp.AD_Org_ID, 
    ...
    c.Title, c.Comments, c.Phone, c.Phone2, c.Fax, c.C_Job_ID, IsFullBPAccess,
...
SELECT l.AD_Client_ID, l.AD_Org_ID, 
    ...
    -- Line
    l.C_BPartner_ID, l.C_BPartner_Location_ID, o.AD_User_ID, bp.C_BP_Group_ID,
    ...
    -- Address
    loc.Address1, loc.Address2, loc.City, loc.Postal, loc.C_Region_ID, loc.C_Country_ID 
FROM C_Order o
...
--  Leads
CREATE OR REPLACE VIEW RV_Lead_Week
AS
SELECT AD_Client_ID, AD_Org_ID,
firstOf(Created, 'DY') AS Created,	--	DD Day, DY Week, MM Month
R_Source_ID, COUNT(*) AS Counter
FROM C_Lead
GROUP BY AD_Client_ID, AD_Org_ID, firstOf(Created, 'DY'), R_Source_ID
/

CREATE OR REPLACE VIEW RV_Lead_Month
AS
SELECT AD_Client_ID, AD_Org_ID,
firstOf(Created, 'MM') AS Created,	--	DD Day, DY Week, MM Month
R_Source_ID, R_Status_ID, COUNT(*) AS Counter
FROM C_Lead
GROUP BY AD_Client_ID, AD_Org_ID, firstOf(Created, 'MM'), R_Source_ID, R_Status_ID
/
...
SELECT C_Payment_ID, AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    ...
    TenderType, CreditCardType, CreditCardNumber, CreditCardExpMM, CreditCardExpYY,
...
SELECT C_Payment_ID, AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    DocumentNo, DateTrx, IsReceipt, C_DocType_ID, TrxType,
    C_BankAccount_ID, C_BPartner_ID, C_Invoice_ID, C_BP_BankAccount_ID, C_PaymentBatch_ID,
    TenderType, CreditCardType, CreditCardExpMM, CreditCardExpYY,
    Micr, RoutingNo, AccountNo, CheckNo,
    A_Name, A_Street, A_City, A_State, A_Zip, A_Ident_DL, A_Ident_SSN, A_EMail,
    VoiceAuthCode, Orig_TrxID, PONum,
    C_Currency_ID, C_ConversionType_ID,
...
--  All to receive Request Updates
...
SELECT i.AD_Org_ID, i.AD_Client_ID, 
	...
	i.DateInvoiced, i.DateAcct, i.C_BP_Group_ID,
    p.NetDays, 
	paymentTermDueDate(i.C_PaymentTerm_ID, i.DateInvoiced) AS DueDate,
...
--  Accounting
    ...
    fa.C_UOM_ID, fa.Qty,
--  Gain/Loss
    gl.AD_PInstance_ID, gl.APAR, gl.OpenAmt, gl.PercentGL,
    ...
    (fa.AmtAcctDr*gl.PercentGL/100) AS AmtAcctOpenDr, (fa.AmtAcctCr*gl.PercentGL/100) AS AmtAcctOpenCr,
    ((fa.AmtAcctDr-fa.AmtAcctCr)*gl.PercentGL/100) AS AmtAcctOpenBalance
FROM T_InvoiceGL gl
...
--
CREATE OR REPLACE VIEW M_ProductLocator_v AS
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    M_Product_ID, M_Locator_ID
FROM M_ProductLocator
WHERE IsActive='Y'
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    M_Product_ID, M_Locator_ID
FROM M_Product
WHERE IsActive='Y' AND M_Locator_ID IS NOT NULL
/

CREATE OR REPLACE VIEW RV_Access
    (AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, AD_Table_ID, Record_ID )
AS
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 403, W_Counter_ID 
FROM W_Counter
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 541, A_Asset_Delivery_ID 
FROM A_Asset_Delivery
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 550, W_Click_ID 
FROM W_Click
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, NULL, 566, AD_Session_ID 
FROM AD_Session
UNION
SELECT  AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, NULL, 615, AD_Registration_ID  
FROM AD_Registration
UNION
SELECT  AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 717, AD_AccessLog_ID 
FROM AD_AccessLog
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, NULL, 828, AD_Issue_ID 
FROM AD_Issue
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 894, CM_WebAccessLog_ID 
FROM CM_WebAccessLog
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 923, C_Lead_ID 
FROM C_Lead
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 923, C_Lead_ID 
...
UNION
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
    Remote_Addr, Remote_Host, AD_User_ID, 904, AD_LdapAccess_ID 
FROM AD_LdapAccess
--UNION
--SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
--    Remote_Addr, Remote_Host, AD_User_ID, 528, R_ContactInterest_ID (No PK ID)
--FROM R_ContactInterest
/

CREATE OR REPLACE VIEW RV_ReceiptNotMatched
AS
SELECT iol.AD_Client_ID, iol.AD_Org_ID, iol.IsActive, iol.Created, iol.CreatedBy, iol.Updated, iol.UpdatedBy,
  io.M_InOut_ID, io.DocumentNo, io.C_DocType_ID, io.DateReceived, io.DateAcct, io.DocStatus,
  io.C_BPartner_ID, io.C_BPartner_Location_ID, io.AD_User_ID,
  iol.M_InOutLine_ID, iol.Line,
  iol.M_Product_ID, iol.MovementQty, COALESCE(m.Qty,0) AS MatchedQty,  
  iol.MovementQty - COALESCE(m.Qty,0) AS NotMatchedQty
FROM M_InOut io
  INNER JOIN M_InOutLine iol ON (io.M_InOut_ID=iol.M_InOut_ID)
  LEFT OUTER JOIN M_MatchInv m ON (m.M_InOutLine_ID=iol.M_InOutLine_ID)
