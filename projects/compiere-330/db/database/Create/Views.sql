...
SELECT bp.AD_Client_ID, bp.AD_Org_ID, 
    ...
    a.C_Country_ID, cc.Name AS CountryName, a.C_Location_ID
FROM C_BPartner bp
...
CREATE OR REPLACE VIEW RV_ProductUOMGroup
AS
SELECT p.Value, p.Name,  
  p.M_Product_Category_ID, p.C_UOM_ID, p.C_UOMGroup_ID,
  uom.C_UOM_To_ID, uom.DivideRate, uom.MultiplyRate,
  grp.C_UOM_ID AS Group_UOM_ID,
  grp.DivideRate AS Group_DivideRate, grp.MultiplyRate AS Group_MultiplyRate,
  CASE WHEN uom.C_UOM_To_ID IS NOT NULL AND grp.C_UOM_ID IS NULL THEN 'P' 
    WHEN uom.C_UOM_To_ID IS NULL AND grp.C_UOM_ID IS NOT NULL THEN 'G' 
    WHEN uom.C_UOM_To_ID IS NOT NULL AND grp.C_UOM_ID IS NOT NULL THEN 'B' 
  END AS GroupStatus
FROM M_Product p
  LEFT OUTER JOIN C_UOM_Conversion uom ON (p.M_Product_ID=uom.M_Product_ID AND p.C_UOM_ID=uom.C_UOM_ID)
  LEFT OUTER JOIN C_UOMGroupConversion grp ON (p.C_UOMGroup_ID=grp.C_UOMGroup_ID AND grp.C_UOM_ID=uom.C_UOM_To_ID)
UNION
SELECT p.Value, p.Name,  
  p.M_Product_Category_ID, p.C_UOM_ID, p.C_UOMGroup_ID,
  NULL, NULL, NULL,
  ...
  'G' AS GroupStatus
FROM M_Product p
  INNER JOIN C_UOMGroupConversion grp ON (p.C_UOMGroup_ID=grp.C_UOMGroup_ID)
WHERE NOT EXISTS (SELECT * FROM C_UOM_Conversion c
  WHERE c.M_Product_ID=p.M_Product_ID 
    AND c.C_UOM_ID=p.C_UOM_ID AND c.C_UOM_TO_ID=grp.C_UOM_ID)
/
