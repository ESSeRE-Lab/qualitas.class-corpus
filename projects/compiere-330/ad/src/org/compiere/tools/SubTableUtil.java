...
public class SubTableUtil
{
	...
	private boolean generateTable() throws Exception
	{
		...
		if (!m_derivedTable.save())
			throw new IllegalStateException("Cannot save " + m_dTableName);
		...
		while (rs.next())
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (columnName.equalsIgnoreCase(dCol.getColumnName()))
				{
					...
					DB.executeUpdateMultiple(sql, false, null);
					found = true;
					list.remove(i);
					break;
				}
		...
		if (noColumns == 0)
		{
			...
			return DB.executeUpdateMultiple(sql, false, null) >= 0;
		}
		//	New Columns
		for (int i = 0; i < list.size(); i++)
		{
	...
	private boolean generateView() throws Exception
	{
		...
		if (!m_viewTable.save())
			throw new IllegalStateException("Cannot save " + m_vTableName);
		...
		if (DB.executeUpdate(sql.toString(), (Trx) null) < 0)
		{
			return false;
		}
		return true;	
	...
	private MColumn[] syncMColumns (boolean derived) throws Exception
	{
		...
		//	tCol.setIsUpdateable(true);
			...
			if (tCol.save())
				throw new IllegalStateException("Cannot save column " + sCol.getColumnName());
		}
		//
		...
		for (MColumn tCol : tCols) {
			...
			if (sCol == null)
			{
				...
				{
					if (!tCol.delete(true))
						throw new IllegalStateException("Cannot delete column " 
								+ tCol.getColumnName());
				}
			}
		}
	...
	private static boolean createColumn (MColumn col, MTable table, boolean alsoInDB)
	{
		...
		boolean success = DB.executeUpdateMultiple(sql, false, table.get_Trx()) >= 0;
		if (success)
