...
/**
 *  Compiere Colored Combo Box.  By default, auto-reduction of choices is on and
 *  is not case sensitive. Editing the number of choices will always be passed
 *  through directly to the unfiltered ComboBoxModel.
 *
 *  @author     Jorg Janke
...
public class CComboBox extends JComboBox implements CEditor
{
	...
	private void init()
	{
		...
		// editable and auto-reducible by default
	 	setEditable(true);
	 	setAutoReducible(true);
		
	 	FIELD_HIGHT = getPreferredSize().height;
	 	
		// will show the complete set of data choices on mouse click of drop
		// down arrow
	...
	/**
	 * Retrieves the complete and unfiltered ComboBoxModel.  Use this when
	 * complete and unfiltered access to all the combo box choices is necessary.
	 * 
	 * @return
	 */
	...
	public void setOpaqueAll (boolean isOpaque)
	{
		...
		if (cbe != null)
		{
			Component cc = cbe.getEditorComponent();
	...
	private class ReducibleModel implements MutableComboBoxModel, ListDataListener 
	{
		...
		{
			return m_listenerList.getListeners(ListDataListener.class);
		}
		...
		public void removeAllElements()
		{
			...
			for (ListDataListener element : listeners)
				removeListDataListener(element);
		...
		public void setFilter(String filter)
		{
			...
			m_lcFilter = this.m_filter.trim().toLowerCase();
		}
		...
		public void updateModel(boolean filtering)
		{
			...
			else
			{
				...
				ListDataListener[] listeners = getListDataListeners();
				for (ListDataListener element2 : listeners)
					removeListDataListener(element2);
				m_model.removeListDataListener(this);
	...
	public static void main(String[] args)
	{
		...
		cp.add(new JLabel("---n---"), BorderLayout.NORTH);
		cp.add(new JLabel("---S---"), BorderLayout.SOUTH);
