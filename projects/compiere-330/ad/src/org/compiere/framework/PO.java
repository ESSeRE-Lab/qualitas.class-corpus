	...
	implements Serializable, Cloneable, Comparator<PO>, Evaluatee
{
	 ...
	 *	@return po or null
	 */
	static public PO copy (Ctx ctx, PO po)
	{
		if (ctx == null || po == null)
			return null;
		try
	...
	/**	Logger							*/
	protected static CLogger	log = CLogger.getCLogger (PO.class);
	/** Static Logger					*/
	...
	public String toString()
	{
		...
		else if (m_KeyColumns != null && m_KeyColumns.length > 1)
		{
			StringBuilder sb = new StringBuilder(getClass().getName()).append("[");
			for (int i = 0; i < m_KeyColumns.length; i++)
            {
	            String keyCol = m_KeyColumns[i];
	            if (i != 0)
	            	sb.append(",");
	 ...
	 * 	@return access level
	 */
	protected String get_AccessLecel()
	{
	...
	public boolean load (Trx trx)
	{
		...
		try
		{
			pstmt = DB.prepareStatement(sql.toString(), m_trx);	//	local p_trx only
			for (int i = 0; i < m_IDs.length; i++)
			{
				Object oo = m_IDs[i];
	...
	private void setKeyInfo()
	{
		...
		for (int i = 0; i < size; i++)
		{
			m_KeyColumns[i] = columnNames.get(i);
	...
	public boolean save()
	{
		...
		//	Organization Check
		String TableTrxType = p_info.getTableTrxType();
		boolean orgRequired = X_AD_Table.TABLETRXTYPE_MandatoryOrganization.equals(TableTrxType);
		if (getAD_Org_ID() == 0)
		{
			if (!orgRequired)
			{
				Boolean shared = MClientShare.isShared(getAD_Client_ID(), get_Table_ID());
		...
		//	Before Save
		MAssignSet.execute(this, newRecord);	//	Auto Value Assignment
		try
		{
	...
	private boolean saveFinish (boolean newRecord, boolean success)
	{
		...
		if (success)
		{
			if (newRecord)
			{
				insertTranslations();
				insertSubTable();
				MRecentItem.add(this, "I");
			}
			else
			{
				updateTranslations();
				updateSubTable();
				MRecentItem.add(this, "U");
			}
			updatePreferences();
		}
		//
	...
	protected boolean saveUpdate()
	{
		...
		if (changes)
		{
			if (m_trx == null)
				...
				log.fine("[" + m_trx + "] - " + p_info.getTableName() + "." + where);
			...
			int no = DB.executeUpdate(sql.toString(), params, false, m_trx);
			boolean ok = no == 1;
			if (ok)
				ok = lobSave();
			else
			{
				if (m_trx == null)
					...
					log.log(Level.WARNING, "#" + no
							+ " - [" + m_trx + "] - " + p_info.getTableName() + "." + where);
			}
	...
	private boolean saveNew()
	{
		...
		if (m_trx == null)
			...
			log.fine("[" + m_trx + "] - " + p_info.getTableName() + " - " + get_WhereClause(true));
	...
	public boolean saveNewInsertSQL()
	{
		...
		//
		int no = DB.executeUpdate(sqlInsert.toString(), params, false, m_trx);
		boolean ok = no == 1;
		if (ok)
		{
			ok = lobSave();
		...
		else
		{
			...
			if (m_trx == null)
				...
				log.log(Level.WARNING, "[" + m_trx + "]" + msg);
		}
		return ok;
	}	//	saveNewInsertSQL
	...
	private boolean insertTranslations()
	{
		...
		/* 	Alternative *
			...
			.append(keyColumn).append("!=t.").append(keyColumn).append(")");
			/** */
		int no = DB.executeUpdate(sql.toString(), new Object[]{get_ID()},false, m_trx);
		log.fine("#" + no);
		return no > 0;
	}	//	insertTranslations
	...
	private boolean updateTranslations()
	{
		...
		int no = DB.executeUpdate(sql.toString(), m_trx);
		log.fine("#" + no);
		return no >= 0;
	}	//	updateTranslations
	...
	public boolean lock()
	{
		...
		if (index != -1)
		{
			...
			boolean success = DB.executeUpdate(sql, (Trx) null) == 1;	//	outside p_trx
			if (success)
				log.fine("success");
			else
	...
	public boolean unlock (Trx trx)
	{
		...
		if (index != -1)
		{
			...
			boolean success = DB.executeUpdate(sql, trx) == 1;
			if (success)
	...
	public void xml_addPO(Node base, boolean dataOnly)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (!dataOnly)
			{
				...
				//
				if (FieldType.isLookup(displayType))		//	should be ID
				{
