...
public class AccessSqlParser
{
	...
	/**	Logger				*/
	private final static CLogger	log = CLogger.getCLogger(AccessSqlParser.class);
	/**	Original SQL			*/
	private String		m_sqlOriginal;
	/**	SQL Selects			*/
	private String[]	m_sql;
	/**	List of Arrays		*/
	private final ArrayList<TableInfo[]>	m_tableInfo = new ArrayList<TableInfo[]>();
	...
	//		log.fine(m_sqlOriginal);
		getSelectStatements();
		//	analyze each select
		for (String element : m_sql)
		{
			TableInfo[] info = getTableInfo(element.trim());
	...
	private TableInfo[] getTableInfo (String sql)
	{
		...
		if (fromIndex != sql.lastIndexOf(FROM))
			log.log(Level.FINE, "More than one FROM clause - " + sql);
		while (fromIndex != -1)
		{
...
//				log.fine("getTableInfo -- " + tableInfo);
	 ...
	 *	@return number of Table Info
	 */
	public int size()
	{
		return m_tableInfo.size();
	}
	...
	public class TableInfo
	{
		...
		private final String m_synonym;
