...
public class PO_Record
{
	 ...
	 */
	static boolean deleteCascade (int AD_Table_ID, int Record_ID, Trx trx)
	{
		//	Table Loop
	 ...
	 */
	static String exists (int AD_Table_ID, int Record_ID, Trx trx)
	{
		//	Table Loop only
		for (int i = 0; i < s_restricts.length; i++)
		{
			...
			StringBuffer sql = new StringBuffer ("SELECT COUNT(*) FROM ")
				...
				.append(" WHERE AD_Table_ID=? AND Record_ID=?");
			int no = DB.getSQLValue(trx, sql.toString(), AD_Table_ID, Record_ID);
			if (no > 0)
