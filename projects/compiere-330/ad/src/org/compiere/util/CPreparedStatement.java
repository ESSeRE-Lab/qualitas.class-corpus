...
public class CPreparedStatement extends CStatement implements PreparedStatement
{
	 ...
	 */
	CPreparedStatement (int resultSetType, int resultSetConcurrency,
		String sql0, Trx trx)
	{
		...
		if( trx == null )
			...
			m_conn = null;
		// redundant logging
		// log.finest("TrxName=" + trx + ",SQL=" + sql1);
	}	//	CPreparedStatement
	...
	protected void finalize() throws Throwable {
		...
		if( m_conn != null && !m_conn.isClosed() ){
			log.log(Level.WARNING, "Connection not properly closed; closing now");
			m_conn.close();
		}
		super.finalize();
	}
