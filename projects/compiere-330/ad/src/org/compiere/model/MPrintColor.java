...
public class MPrintColor extends X_AD_PrintColor
{
	 ...
	 */
	public MPrintColor (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}	//	MPrintColor
	...
	/** Cached Colors						*/
	static private final CCache<Integer,MPrintColor> 	s_colors = new CCache<Integer,MPrintColor>("AD_PrintColor", 20);
	/**	Static Logger	*/
	private static final CLogger	s_log	= CLogger.getCLogger (MPrintColor.class);
	...
	//		return new MPrintColor (ctx, 0);
		...
		MPrintColor pc = s_colors.get(ctx, key);
		if (pc == null)
		{
	 ...
	 *  @param trx transaction
	 */
	public MPrintColor(Ctx ctx, int AD_PrintColor_ID, Trx trx)
	{
		super (ctx, AD_PrintColor_ID, trx);
		if (AD_PrintColor_ID == 0)
			setIsDefault(false);
	}	//	MPrintColor
...
**/
		...
		for (int element : IDs) {
			MPrintColor pc = new MPrintColor(Env.getCtx(), element, null);
			System.out.println(element + ": " + pc + " = " + pc.getColor() + ", RGB=" + pc.getColor().getRGB());
		}
	}	//	main
