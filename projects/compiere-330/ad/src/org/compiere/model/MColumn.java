...
public class MColumn extends X_AD_Column
{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (!newRecord)
		{
			...
			int no = DB.executeUpdate(sql.toString(), get_Trx());
			...
			else
			{
				...
				else
				{
					if (FieldType.isText(dt)
						...
						|| DisplayTypeConstants.Button == dt && !columnName.endsWith("_ID")
	...
	public boolean isFK()
	{
		...
		if (FieldType.isID (dt)
			...
			&& !isVirtualColumn())
			return true;
		if (dt == DisplayTypeConstants.Button && getColumnName().endsWith ("_ID"))
			return true;
		return false;
	}	//	isFK
