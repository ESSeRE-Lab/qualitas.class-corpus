...
public class TranslationTable
{
	...
	{
		TranslationTable retValue = s_cache.get (null, baseTableName);
		if (retValue != null)
			return retValue;
	...
	/**	Cache						*/
	private static final CCache<String,TranslationTable> s_cache = new CCache<String,TranslationTable>("TranslationTable", 20);
	...
	public boolean createTranslation (PO po)
	{
		...
		{
			String columnName = m_columns.get(i);
		...
		int no = DB.executeUpdate(sql.toString(), po.get_Trx());
	...
	public boolean resetTranslationFlag (PO po)
	{
		...
		int no = DB.executeUpdate(sb.toString(), po.get_Trx());
		log.fine(m_trlTableName + ": ID=" + po.get_ID() + " #" + no);
		return no != 0;
