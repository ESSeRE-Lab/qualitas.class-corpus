...
public class MElementValue extends X_C_ElementValue
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
			...
			&& is_ValueChanged("IsSummary"))
		{
			...
			int no = DB.getSQLValue(get_Trx(), sql, getC_ElementValue_ID());
			if (no != 0)
			{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		{
			MAccount.updateValueDescription(getCtx(), "Account_ID=" + getC_ElementValue_ID(),get_Trx());
			if ("Y".equals(getCtx().getContext("$Element_U1"))) 
				MAccount.updateValueDescription(getCtx(), "User1_ID=" + getC_ElementValue_ID(),get_Trx());
			if ("Y".equals(getCtx().getContext("$Element_U2"))) 
				MAccount.updateValueDescription(getCtx(), "User2_ID=" + getC_ElementValue_ID(),get_Trx());
		}
