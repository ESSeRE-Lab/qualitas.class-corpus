...
implements Comparator<PO>, Serializable
{
	...
	{
		synchronized(s_countries) {
			if (s_countries.isEmpty())
				loadAllCountries(ctx);
		}
		String key = String.valueOf(C_Country_ID);
		MCountry c = s_countries.get(ctx, key);
		if (c != null)
			return c;
	...
	/**	Country Cache					*/
	private static final CCachePerm<String,MCountry>	s_countries = new CCachePerm<String,MCountry>("C_Country", 250);
	/**	Default Country 				*/
	 ...
	 */
	public MCountry (Ctx ctx, int C_Country_ID, Trx trx)
	{
	 ...
	 *	@param trx transaction
	 */
	public MCountry (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}	//	MCountry
	...
	public boolean isValidRegion(int C_Region_ID)
	{
		...
		for (MRegion element : regions) {
			if (C_Region_ID == element.getC_Region_ID())
				return true;
		}
		return false;
	}	//	isValidRegion
