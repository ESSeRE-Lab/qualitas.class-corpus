...
public class MAccount extends X_C_ValidCombination
{
	...
	public static MAccount getDefault (Ctx ctx, int C_AcctSchema_ID, 
		boolean optionalNull, Trx trx)
	{
		MAcctSchema acctSchema = new MAcctSchema (ctx, C_AcctSchema_ID, trx);
	 ...
	 *	@param trx transaction
	 */
	public static void updateValueDescription (Ctx ctx, String where, Trx trx)
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				MAccount account = new MAccount (ctx, rs, trx);
	 ...
	 *	@param trx transaction
	 */
	public MAccount (Ctx ctx, int C_ValidCombination_ID, Trx trx)
	{
	 ...
	 *	@param trx transaction
	 */
	public MAccount (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}   //  MAccount
	...
	public MElementValue getAccount ()
	{
		...
		{
			if (getAccount_ID() != 0)
				m_accountEV = new MElementValue(getCtx(), getAccount_ID(), get_Trx());
		}
		return m_accountEV;
	}	//	setAccount
	...
	public void setValueDescription()
	{
		...
		//
		MAcctSchema as = new MAcctSchema(getCtx(), getC_AcctSchema_ID(), get_Trx());	//	In Trx!
		...
		for (int i = 0; i < elements.length; i++)
		{
			...
			if (X_C_AcctSchema_Element.ELEMENTTYPE_Organization.equals(element.getElementType()))
			{
				if (getAD_Org_ID() != 0)
				{
					MOrg org = new MOrg(getCtx(), getAD_Org_ID(), get_Trx());	//	in Trx!
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_Account.equals(element.getElementType()))
			{
				if (getAccount_ID() != 0)
				{
					if (m_accountEV == null)
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_SubAccount.equals(element.getElementType()))
			{
				if (getC_SubAcct_ID() != 0)
				{
					X_C_SubAcct sa = new X_C_SubAcct(getCtx(), getC_SubAcct_ID(), get_Trx());
					combiStr = sa.getValue();
					descrStr = sa.getName();
				}
			}
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_Product.equals(element.getElementType()))
			{
				if (getM_Product_ID() != 0)
				{
					X_M_Product product = new X_M_Product (getCtx(), getM_Product_ID(), get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_BPartner.equals(element.getElementType()))
			{
				if (getC_BPartner_ID() != 0)
				{
					X_C_BPartner partner = new X_C_BPartner (getCtx(), getC_BPartner_ID(),get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_OrgTrx.equals(element.getElementType()))
			{
				if (getAD_OrgTrx_ID() != 0)
				{
					MOrg org = new MOrg(getCtx(), getAD_OrgTrx_ID(), get_Trx());	// in Trx!
					combiStr = org.getValue();
					descrStr = org.getName();
				}
				else if (element.isMandatory())
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_LocationFrom.equals(element.getElementType()))
			{
				if (getC_LocFrom_ID() != 0)
				{
					MLocation loc = new MLocation(getCtx(), getC_LocFrom_ID(), get_Trx());	//	in Trx!
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_LocationTo.equals(element.getElementType()))
			{
				if (getC_LocTo_ID() != 0)
				{
					MLocation loc = new MLocation(getCtx(), getC_LocFrom_ID(), get_Trx());	//	in Trx!
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_SalesRegion.equals(element.getElementType()))
			{
				if (getC_SalesRegion_ID() != 0)
				{
					MSalesRegion loc = new MSalesRegion(getCtx(), getC_SalesRegion_ID(), get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_Project.equals(element.getElementType()))
			{
				if (getC_Project_ID() != 0)
				{
					X_C_Project project = new X_C_Project (getCtx(), getC_Project_ID(), get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_Campaign.equals(element.getElementType()))
			{
				if (getC_Campaign_ID() != 0)
				{
					X_C_Campaign campaign = new X_C_Campaign (getCtx(), getC_Campaign_ID(), get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_Activity.equals(element.getElementType()))
			{
				if (getC_Activity_ID() != 0)
				{
					X_C_Activity act = new X_C_Activity (getCtx(), getC_Activity_ID(), get_Trx());
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_UserList1.equals(element.getElementType()))
			{
				if (getUser1_ID() != 0)
				{
					MElementValue ev = new MElementValue(getCtx(), getUser1_ID(), get_Trx());
					combiStr = ev.getValue();
					descrStr = ev.getName();
				}
			}
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_UserList2.equals(element.getElementType()))
			{
				if (getUser2_ID() != 0)
				{
					MElementValue ev = new MElementValue(getCtx(), getUser2_ID(), get_Trx());
					combiStr = ev.getValue();
					descrStr = ev.getName();
				}
			}
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_UserElement1.equals(element.getElementType()))
			{
				if (getUserElement1_ID() != 0)
				{
				}
			}
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_UserElement2.equals(element.getElementType()))
			{
				if (getUserElement2_ID() != 0)
				{
				}
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord || is_ValueChanged("Alias"))
		{
			String sql = "SELECT COUNT(*) FROM C_ValidCombination WHERE Alias=?";
			int ii = DB.getSQLValue(null, sql, getAlias());
			if (ii != 0)
			{
				log.saveError("Error", Msg.getMsg(getCtx(), "DuplicateAlias"));;
				return false;
			}
		}
