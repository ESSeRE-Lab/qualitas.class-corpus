 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MTreeNode.java,v 1.3 2006/07/30 00:58:37 jjanke Exp $
 */
public class MTreeNode extends X_AD_TreeNode
{
	/**
	 * 
	 */
	...
	public static MTreeNode get (MTree tree, int Node_ID)
	{
		...
		try
		{
			...
			if (rs.next ())
				retValue = new MTreeNode (tree.getCtx(), rs, tree.get_Trx());
			rs.close ();
			pstmt.close ();
			pstmt = null;
		}
	 ...
	 */
	public MTreeNode (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}	//	MTreeNode
