...
public class MAttachmentEntry
{
	...
	static byte[] getData(URI uri)
	{
		...
		if ("file".equals(schema))
		{
			...
			if (!file.exists())
			{
				s_log.warning("File does not exist: " + uri);
				return null;
			}
			try
			{
			...
			{
				s_log.log(Level.WARNING, "(file) " + uri, e);
				return null;
			}
		}
		...
		{
			s_log.log(Level.WARNING, "(url) " + uri, e);
		}
		return null;
	}	//	getData

	/**	Logger	*/
    private static CLogger s_log = CLogger.getCLogger(MAttachmentEntry.class);
	...
	/**
	 * 	Get Data.
	 *	if reference: load from reference
	...
	/**
	 * 	Get Data.
	 *	@param raw if false and reference: load from reference
	 *	@return Returns the data.
	 */
	public byte[] getData(boolean raw)
	{
		if (raw)
			return m_data;
		//
	...
	public URI getURI()
	{
		//	Assumption that reference is < 200 bytes
		if (m_data == null || m_data.length > 200)
			return null;
		
		try
		{
			String ss = new String(m_data);
			URI uri = new URI(ss);
			return uri;
		}
		catch (Exception e)
	...
	{
		return m_name.toLowerCase().endsWith(".gif") || m_name.toLowerCase().endsWith(".jpg") 
			|| m_name.toLowerCase().endsWith(".png");
	}	//	isGraphic
	...
	{
		byte[] data = getData(false);	//	de-reference
		if (data == null)
			return null;
