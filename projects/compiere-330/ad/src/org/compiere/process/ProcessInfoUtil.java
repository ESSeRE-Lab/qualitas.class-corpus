	...
	public static void saveLogToDB (ProcessInfo pi)
	{
		...
		for (ProcessInfoLog element : logs) 
		{
			...
			sql.append(pi.getAD_PInstance_ID()).append(",")
				.append(element.getLog_ID()).append(",");
			if (element.getP_Date() == null)
				sql.append(DB.NULL("I", Types.TIMESTAMP));
			else
				sql.append(DB.TO_DATE(element.getP_Date(), false));
			sql.append(",");
			if (element.getP_ID() == 0)
				sql.append(DB.NULL("I", Types.INTEGER));
			else
				sql.append(element.getP_ID());
			sql.append(",");
			if (element.getP_Number() == null)
				sql.append(DB.NULL("I", Types.DECIMAL));
			else
				sql.append(element.getP_Number());
			sql.append(",");
			if (element.getP_Msg() == null)
				sql.append("NULL)");
			else
				sql.append(DB.TO_STRING(element.getP_Msg(),2000)).append(")");
			//
			DB.executeUpdate(sql.toString(), (Trx) null);
		}
	}   //  saveLogToDB
