 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: CompiereTabbedPaneUI.java,v 1.3 2006/07/30 00:52:23 jjanke Exp $
 */
public class CompiereTabbedPaneUI extends MetalTabbedPaneUI
{
	...
	//	System.out.println("Tab " + tabIndex + " Comp=" + comp.getName() + " " + comp.getClass().getName() + " x=" + x + ", y=" + y + ", w=" +w + ", h=" + h);
		...
		if( !isSelected )
		{
			Shape top = new Rectangle (x, y, w, h);          //  upper half
			if (tabPlacement == TOP || tabPlacement == LEFT)
				top = new Polygon (     //  top left triangle
					new int[] {x+6, x+w, x+w,     x,       x   },
					new int[] {y,   y,   y+h, y+h, y+6 }, 5);
			else if (tabPlacement == RIGHT)
				top = new Polygon (     //  top right triangle
					new int[] {x, x+w-6, x+w, x+w,     x       },
					new int[] {y, y,     y+6, y+h, y+h }, 5);
			GradientPaint paint = new GradientPaint (
					x, y, new Color(0,0,0,32),
					x, y+h, new Color(0,0,0,64) );
			g2D.setPaint(paint);
			g2D.fill(top);			
		}
...
//				|| (selRect.x - 1 > w)      //  bug !!
		...
		int x, int y, int w, int h, int btm, int rght, boolean isSelected)
	{
		...
		lastTabInRun( tabCount, currentRun );
		g.translate( x, y );
		...
		Rectangle tabRect, Rectangle iconRect, Rectangle textRect, boolean isSelected)
	{
		...
		SwingUtilities.layoutCompoundLabel(tabPane,
			metrics, title, icon,
			SwingConstants.TOP,     //  vert
			SwingConstants.LEFT,    //  horiz
			SwingConstants.CENTER,      //  vert Text
			SwingConstants.TRAILING,    //  horiz Text
			tabRect,
			iconRect,
			textRect,
			textIconGap);
