...
public class Product
{
		...
		throws SQLException
	{
		...
		{
			price = Compiere.ZERO;
			sql = "SELECT bp.M_ProductBOM_ID, bp.BOMQty, p.IsBOM "
		...
		throws SQLException
	{
		return bomQty(p_M_Product_ID, p_M_Warehouse_ID, p_M_Locator_ID, "QtyOnHand", "Y")
			.subtract(bomQty(p_M_Product_ID, p_M_Warehouse_ID, p_M_Locator_ID, "QtyReserved", "Y"));
	}	//	bomQtyAvailable
		...
		throws SQLException
	{
		...
		while (rs.next())
		{
			...
			isVerified = "Y".equals(rs.getString(5));
			ProductType = rs.getString(6);
			
			//	Stocked Items "leaf node"
			if (ProductType.equals("I") && isStocked)
			{
				//	Get ProductQty
				productQuantity = getStorageQty(M_ProductBOM_ID, M_Warehouse_ID, p_M_Locator_ID, p_what, p_CheckATP);
			...
			{
				productQuantity = bomQty (M_ProductBOM_ID, M_Warehouse_ID, p_M_Locator_ID, p_what, p_CheckATP);
				//	How much can we make overall
				if (productQuantity.compareTo(quantity) < 0)
					quantity = productQuantity;
			}
