 ...
 */
public class Payment extends UDF
{
	/**
	 ...
	 *	@throws SQLException
	 */
	public static double  allocated (int p_C_Payment_ID, int p_C_Currency_ID)
		throws SQLException
	{
		...
		String sql = "SELECT C_Charge_ID "
			+ "FROM C_Payment "
			+ "WHERE C_Payment_ID=?";
		int C_Charge_ID = Compiere.getInstance().getSQLValue(sql, p_C_Payment_ID);
		if (C_Charge_ID > 0)
		...
		throws SQLException
	{
		...
		while (rs.next())
		{
			...
			//
			BigDecimal allocation = BigDecimal.valueOf(Currency.convert(amount, //.multiply(MultiplierAP),
