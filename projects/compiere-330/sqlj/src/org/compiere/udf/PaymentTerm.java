 ...
 */
public class PaymentTerm extends UDF
{
	/**
	 * 	Get Due Days.
	 ...
	 *	@throws SQLException
	 */
	public static double  discountD (double p_Amount, int p_C_Currency_ID, 
			int p_C_PaymentTerm_ID,
			Timestamp p_DocDate, Timestamp p_PayDate)
			throws SQLException
		{
			return discount (new BigDecimal(p_Amount),  p_C_Currency_ID, 
				 p_C_PaymentTerm_ID,
				 p_DocDate,  p_PayDate);
		}
	
	public static double  discount (BigDecimal p_Amount, int p_C_Currency_ID, 
			int p_C_PaymentTerm_ID,
			Timestamp p_DocDate, Timestamp p_PayDate)
			throws SQLException
		{
		...
		if (rs.next())
		{
			...
			if (discount.signum() != 0)
			{
				...
				discount = BigDecimal.valueOf(Currency.round(discount, p_C_Currency_ID, "N"));//jz
			}
		}	
		rs.close();
		pstmt.close();
