...
public class TableElement extends PrintElement
{
	...
	/** Print Data				*/
	private ArrayList<ArrayList<ArrayList<Object>>> m_printRows 
		= new ArrayList<ArrayList<ArrayList<Object>>>();
				...
				if (m_columnMaxWidth[col] != 0 && m_columnMaxWidth[col] != -1)
				{
					...
					{ 
						...
						for (String element : lines) {
							AttributedString aString = new AttributedString(element);
		...
		else
		{
			...
			for (int col = 0; col < m_columnWidths.size(); col++)
			{
				...
				if (availableWidth < columnWidth)
				{
					...
					{
						float repColumnWidth = m_columnWidths.get(repCol).floatValue();
	...
	private int getRow (int yPos, int pageNo)
	{
		...
		{
			int rowHeight = m_rowHeights.get(row).intValue();	//	includes 2*Gaps+Line
	...
	private int getCol (int xPos, int pageNo)
	{
		...
		{
			int colWidth = m_columnWidths.get(col).intValue();		//	includes 2*Gaps+Line
		...
		final int firstRow, final int nextPageRow, boolean isView)
	{
		...
		String columnName = Query.getZoomColumnName(m_columnHeader[col].getID());
		String tableName = Query.getZoomTableName(columnName).toUpperCase();
		int AD_Table_ID = Query.getAD_TableID(tableName);
		...
		if (m_columnHeader[col].toString().length() > 0)
		{
			...
			while (measurer.getPosition() < iter.getEndIndex())		//	print header
			{
				...
				{
					if (alignment.equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Block))
					{
					...
					float penX = curX;
					if (alignment.equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Center))
						penX += (netWidth-layout.getAdvance())/2;
					else if ((alignment.equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight) && layout.isLeftToRight())
						|| (alignment.equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft) && !layout.isLeftToRight()))
		...
		for (int row = firstRow; row < nextPageRow; row++)
		{
			...
			float penY = curY;
			for (Object element : printItems) {
				if (element == null)
					;
				else if (element instanceof ImageElement)
				{
					ImageElement m_image = ((ImageElement)element);
					
					if (m_image.isImageNotLoaded()){
							g2D.drawImage(m_image.getImage(),
										curX, (int)penY, this);
					}
				}
				else if (element instanceof BarcodeElement)
				{
					((BarcodeElement)element).getBarcode().draw(g2D, curX, (int)penY);
				}
				...
				else if (element instanceof HTMLRenderer)
				{
					HTMLRenderer renderer = (HTMLRenderer)element;
				...
				else
				{
					...
					if (str.length() > 0)
					{
						...
						for (int lineNo = 0; lineNo < lines.length; lineNo++)
						{
							...
							String packageDefinition=System.getProperty("package.definition", "");
						    boolean isPdfLinkEnabled=false;
						    
						    // for web applications only, enable pdf links
						    if (packageDefinition.contains("apache")){
						    	isPdfLinkEnabled=true;
						    }
							if ((isView || isPdfLinkEnabled) && 
									element instanceof NamePair)	//	ID
							{
							...
							while (measurer.getPosition() < iter.getEndIndex())		//	print element
							{
								...
								{
									if (alignment.equals(X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Block) && measurer.getPosition() < iter.getEndIndex())
									{
										layout = layout.getJustifiedLayout(netWidth + 2);
										fastDraw = false;
									}
									...
									{	//	Bug - set Font/Color explicitly
										...
										if (isView && element instanceof NamePair)	//	ID
										{
										...
										{											
											...
											if (pp instanceof KeyNamePair){
												code = new Integer(((KeyNamePair)pp).getKey());
												Record_ID = ((Integer)code).intValue();
											}
											
											int AD_Window_ID = Query.getZoomAD_Window_ID(AD_Table_ID, Record_ID);
																					
											g2D.setRenderingHint(PdfGraphics2D.HyperLinkKey.KEY_INSTANCE, "Compiere.html#Report?columnName="+columnName+"&Record_ID="+Record_ID+"&windowID="+AD_Window_ID+"&type=zoom&mode=single");
											g2D.drawString(iter, penX, penY);
											g2D.setRenderingHint(PdfGraphics2D.HyperLinkKey.KEY_INSTANCE, PdfGraphics2D.HyperLinkKey.VALUE_HYPERLINKKEY_OFF);											
										}
										else
										{
											g2D.drawString(iter, penX, penY);		
