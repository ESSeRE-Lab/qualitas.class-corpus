...
public abstract class PrintElement implements ImageObserver
{
		...
		String FieldAlignmentType)
	{
		...
		if ((p_FieldAlignmentType == null) || (p_FieldAlignmentType == X_AD_PrintFormatItem.FIELDALIGNMENTTYPE_Default))
	 ...
	 */
	public abstract void paint (Graphics2D g2D, int pageNo, Point2D pageStart, Ctx ctx, boolean isView);
	...
	public boolean waitForLoad (Image image)
	{
		...
		try
		{
			...
			while (!toolkit.prepareImage(image, -1, -1, this))	//	ImageObserver calls imageUpdate
			{
				...
				{
					log.log(Level.WARNING, "Interrupted " + p_info, ex);
					break;
				}
				count++;
			}
