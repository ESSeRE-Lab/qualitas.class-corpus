...
public class PrintDataGroup
{
	 ...
	 * 	@return null if no group change otherwise old value
	 */
	public boolean checkChange (String groupColumnName, Object value)
	{
		if (!isGroupColumn(groupColumnName))
			return false;
	 ...
	 * 	@return null if no group change otherwise old value
	 */
	public Object getGroupColumnValue (String groupColumnName)
	{
		if (!isGroupColumn(groupColumnName))
			return null;
	 ...
	 * 	@return null if no group change otherwise old value
	 */
	public void setGroupColumnValue (String groupColumnName, Object newValue)
	{
		if (!isGroupColumn(groupColumnName))
			return;
		//
	...
	public char[] getFunctions(String columnName)
	{
		...
		while(it.hasNext())
		{
			String group_function = it.next();	//	=TOTAL=~LoadSeq
			if (group_function.startsWith(columnName))
			{
				...
				for (int i = 0; i < m_functions.size(); i++)
				{
					String col_function = (m_functions.get(i));	//	LoadSeq~A
		...
		for (int i = 0; i < retValue.length; i++)
			retValue[i] = (list.get(i)).charAt(0);
	...
	public void addValue (String functionColumnName, BigDecimal functionValue)
	{
		...
		for (int i = 0; i < m_groups.size(); i++)
		{
			String groupColumnName = m_groups.get(i);
			String key = groupColumnName + DELIMITER + functionColumnName;
			PrintDataFunction pdf = m_groupFunction.get(key);
			if (pdf == null)
	...
	public String toString (boolean withData)
	{
		...
		if (withData)
		{
			...
			{
				String key = it.next();
				PrintDataFunction value = m_groupFunction.get(key);
				sb.append(":").append(key).append("=").append(value);
			}
		}
		sb.append("]");
