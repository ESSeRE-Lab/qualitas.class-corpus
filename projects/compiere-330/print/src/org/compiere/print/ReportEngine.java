...
queued-job-count = 0  (class javax.print.attribute.standard.QueuedJobCount)
	...
	public boolean createPDF (File file)
	{
		...
		try
		{
			...
			CompierePdfGraphics2D.writePDF(out, layout);
	...
	//	String JobName = DOC_BASETABLES[type] + "_Print";
		...
		if (type == CHECK)
        	...
        	sql = " SELECT dt.DocBaseType, pf.RPL_TList_PrintFormat_ID, pf.PUT_TList_PrintFormat_ID, pf.PCK_CluTList_PrintFormat_ID, "
        		+ " pf.PCK_OrdTList_PrintFormat_ID, M.PickMethod, "
        		+ " c.IsMultiLingualDocument, COALESCE(dt.DocumentCopies,0) "
        		+ " FROM M_TaskList M "
	...
	public byte[] getPDFAsArray(LayoutEngine layout){
		...
		try {
			CompierePdfGraphics2D.writePDF(bout, layout);
