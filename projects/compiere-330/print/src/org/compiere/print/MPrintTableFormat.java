...
public class MPrintTableFormat extends X_AD_PrintTableFormat
{
	 ...
	 */
	public MPrintTableFormat (Ctx ctx, int AD_PrintTableFormat_ID, Trx trx)
	{
	 ...
	 *  @param trx transaction
	 */
	public MPrintTableFormat (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}	//	MPrintTableFormat
	...
	public Font getHeader_Font()
	{
		...
		if (i != 0)
			header_Font = MPrintFont.get(getCtx(), i).getFont();
		if (header_Font == null)
	...
	public Font getFunct_Font()
	{
		...
		if (i != 0)
			funct_Font = MPrintFont.get(getCtx(), i).getFont();
		if (funct_Font == null)
	...
	private static final CCache<Integer,MPrintTableFormat>	s_cache
		= new CCache<Integer,MPrintTableFormat>("AD_PrintTableFormat", 3);
	/** Static Logger					*/
	private static final CLogger 	s_log = CLogger.getCLogger(MPrintTableFormat.class);
	...
	static public MPrintTableFormat get (Ctx ctx, int AD_PrintTableFormat_ID, Font standard_font)
	{
		...
		MPrintTableFormat tf = s_cache.get(ctx, ii);
		if (tf == null)
		{
			if (AD_PrintTableFormat_ID == 0)
				tf = getDefault (ctx);
	...
	{
		return get (ctx, AD_PrintTableFormat_ID, MPrintFont.get (ctx, AD_PrintFont_ID).getFont());
	}	//	get
