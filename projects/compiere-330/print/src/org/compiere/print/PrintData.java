...
public class PrintData implements Serializable
{
	...
	public void addRow (boolean functionRow, int levelNo)
	{
		...
		if (m_hasLevelNo && levelNo != 0)
			addNode(new PrintDataElement(LEVEL_NO, new Integer(levelNo), DisplayTypeConstants.Integer));
	}	//	addRow
	...
	public int getIndex (int AD_Column_ID)
	{
		if (m_columnInfo == null)
			return -1;
		for (PrintDataColumn element : m_columnInfo) {
			if (element.getAD_Column_ID() == AD_Column_ID)
				return getIndex(element.getColumnName());
		}
			...
			for (int i = 0; i < pd.getNodeCount(); i++)
			{
				...
				else if (o instanceof PrintDataElement)
				{
					...
					if (!pde.isNull())
					{
						...
						element.appendChild(document.createTextNode(pde.getValueDisplay(null, -1)));	//	not formatted
						row.appendChild(element);
					}
				}
				else
	...
	public static void main(String[] args)
	{
		...
		//	parse
		System.out.println("");
		PrintData pd1 = parseXML (new Ctx(), new File("C:\\Temp\\printData.xml"));
