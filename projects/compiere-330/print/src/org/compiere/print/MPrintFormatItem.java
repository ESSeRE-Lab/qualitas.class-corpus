...
public class MPrintFormatItem extends X_AD_PrintFormatItem
{
	 ...
	 */
	public MPrintFormatItem (Ctx ctx, int AD_PrintFormatItem_ID, Trx trx)
	{
	 ...
	 *	@param trx transaction
	 */
	public MPrintFormatItem (Ctx ctx, ResultSet rs, Trx trx)
	{
		super(ctx, rs, trx);
	}	//	MPrintFormatItem
	...
	private static final CCache<Integer,String>	s_columns = new CCache<Integer,String>("AD_PrintFormatItem", 200);
	...
	private static String getColumnName (Integer AD_Column_ID)
	{
		if (AD_Column_ID == null || AD_Column_ID.intValue() == 0)
			return null;
		//
		String retValue = s_columns.get(null,AD_Column_ID);
		if (retValue == null)
		{
	...
	public static MPrintFormatItem createFromColumn (MPrintFormat format, int AD_Column_ID, int seqNo)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				else if (displayType == DisplayTypeConstants.Text || displayType == DisplayTypeConstants.Memo )
				...
				if (isKey
					...
					|| displayType == DisplayTypeConstants.Button || displayType == DisplayTypeConstants.Binary
					|| displayType == DisplayTypeConstants.ID || displayType == DisplayTypeConstants.Image
					|| displayType == DisplayTypeConstants.RowID
					|| seqNo == 0 )
				{
			...
			*/
			String sql = "UPDATE AD_PrintFormatItem_Trl "
				...
				+ " AND EXISTS (SELECT * "
					...
					+ " AND e.AD_Element_ID=c.AD_Element_ID"
					+ " AND c.AD_Column_ID=?"
					+ " AND AD_PrintFormatItem_Trl.AD_PrintFormatItem_ID = ?)"
				+ " AND EXISTS (SELECT * FROM AD_Client "
					+ "WHERE AD_Client_ID=AD_PrintFormatItem_Trl.AD_Client_ID AND IsMultiLingualDocument='Y')";
			int no = DB.executeUpdate(sql, new Object[]{getAD_Column_ID(), get_ID(), getAD_Column_ID(), get_ID()}, false, get_Trx());
	...
	public static MPrintFormatItem createFromBViewColumn(MPrintFormat format,
			int AD_Column_ID, int AD_BView_ID, int seqNo) {
		MPrintFormatItem pfi = new MPrintFormatItem(format.getCtx(), 0, null);
		pfi.setAD_PrintFormat_ID(format.getAD_PrintFormat_ID());
		pfi.setClientOrg(format);
		pfi.setAD_Column_ID(AD_Column_ID);
		...
		// translation is dome by trigger
		String sql = "SELECT c.ColumnName,bvf.Name,bvf.Name, " // 1..3
				+ "c.AD_Reference_ID,c.IsKey,bvf.SeqNo " // 4..6
				+ ",bvf.IsPrinted "
				+ "FROM AD_Column c, AD_BView_Source bvs, AD_BView_Source_Mapping bvsm, AD_BView_Field bvf "
				+ "WHERE c.AD_Column_ID=?"
				+ " AND bvsm.AD_BView_Source_Field_ID = c.AD_Column_ID AND bvsm.AD_BView_Source_ID = bvs.AD_BView_Source_ID and bvs.IsDefault = 'Y' AND bvs.AD_BView_ID = ? AND bvsm.AD_BView_Field_ID = bvf.AD_BView_Field_ID";
		// translate base entry if single language - trigger copies to trl
		// tables
		Language language = format.getLanguage();
		boolean trl = !Env.isMultiLingualDocument(format.getCtx())
		...
		try {
			PreparedStatement pstmt = DB.prepareStatement(sql, (Trx) null);
			pstmt.setInt(1, AD_Column_ID);
			pstmt.setInt(2, AD_BView_ID);
			if (trl)
				pstmt.setString(3, language.getAD_Language());
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				pfi.setName(rs.getString(2));
				pfi.setPrintName(rs.getString(3));
				...
				if (FieldType.isNumeric(displayType))
					pfi.setFieldAlignmentType(FIELDALIGNMENTTYPE_TrailingRight);
						...
						|| displayType == DisplayTypeConstants.Memo)
					pfi.setFieldAlignmentType(FIELDALIGNMENTTYPE_Block);
				else
					pfi.setFieldAlignmentType(FIELDALIGNMENTTYPE_LeadingLeft);
				boolean isPrinted = "Y".equals(rs.getString(7));
				//
				...
				s_log.log(Level.SEVERE, "Not Found AD_Column_ID="
						+ AD_Column_ID + " Trl=" + trl + " "
