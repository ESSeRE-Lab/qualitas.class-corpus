...
public class Template extends CO {
	...
	public MTemplate getCM_Template(String ID, int CM_WebProject_ID) {
		if (cache.containsKey(ID)) {
			...
			int[] tableKeys = PO.getAllIDs("CM_Template", "CM_Template_ID=" + ID + " AND CM_WebProject_ID=" + CM_WebProject_ID, HttpServletCM.getTrx());
			if (tableKeys.length==0) {
				// TODO Throw Template Exception
				return null;
			} else if (tableKeys.length==1) {
				MTemplate thisTemplate = new MTemplate(ctx, tableKeys[0], HttpServletCM.getTrx());
