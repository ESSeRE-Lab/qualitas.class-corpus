				...
				if (wi.getUser_ID ()>0) 
				{
					...
					if (theseBPBankAccount!=null && theseBPBankAccount.length>0)
					{
						for (int i=0; i<theseBPBankAccount.length;i++) 
						{
							...
							if (theseBPBankAccount[i].getBank ()!=null) 
								xmlCode = theseBPBankAccount[i].getBank ().get_xmlString (xmlCode);
						}
					}
		...
		Chat chatCache)
	{
		...
		if (thisRequest.isStage ())
		{
			...
			if (tableKeys.length > 0)
			{
				...
				for (int i = 0; i < tableKeys.length; i++)
				{
					X_CM_CStage_Element thisElement = new X_CM_CStage_Element(thisRequest.getCtx(), tableKeys[i], null);
					if (thisElement != null)
					{
		...
		if (l_nTableIDs.length > 0) {
			...
			if (completeXML) {
				for (int i = 0; i < l_nIDs.length; i++) {
					l_Object = table.getPO(ctx, l_nIDs[i], l_szTrxName);
		...
		if (tableKeys.length > 0)
		{
			...
			for (int i = 0; i < tableKeys.length; i++)
			{
				X_CM_TemplateTable thisTemplateTable = new X_CM_TemplateTable (
					ctx, tableKeys[i], HttpServletCM.getTrx());
				try
				{
		...
		if (theseChannels != null && theseChannels.length > 0)
		{
			for (int i = 0; i < theseChannels.length; i++)
			{
				MNewsChannel thisChannel = new MNewsChannel (ctx,
					theseChannels[i], HttpServletCM.getTrx());
