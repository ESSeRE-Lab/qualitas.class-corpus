...
public final class VBPartner extends CDialog implements ActionListener
{
	...
	private void initBPartner()
	{
		...
		if (!ro)
			ro = !MRole.getDefault().canUpdate(
				Env.getCtx().getAD_Client_ID(), Env.getCtx().getAD_Org_ID(), 
				X_C_BPartner_Location.Table_ID, 0, false);
		if (!ro)
			ro = !MRole.getDefault().canUpdate(
				Env.getCtx().getAD_Client_ID(), Env.getCtx().getAD_Org_ID(), 
				X_C_Location.Table_ID, 0, false);
	...
	public boolean loadBPartner (int C_BPartner_ID)
	{
		...
		m_pLocation = m_partner.getLocation(
			Env.getCtx().getContextAsInt( m_WindowNo, "C_BPartner_Location_ID"));
		if (m_pLocation != null)
		{
		...
		m_user = m_partner.getContact(
			Env.getCtx().getContextAsInt( m_WindowNo, "AD_User_ID"));
		if (m_user != null)
		{
	...
	private boolean actionSave()
	{
		...
		if (value == null || value.length() == 0)
		{
			...
			value = DB.getDocumentNo (Env.getCtx().getAD_Client_ID(), "C_BPartner", null);
