...
public class HTMLEditor extends CDialog
{
	...
	/**	Logger				*/
	private static CLogger	log = CLogger.getCLogger(HTMLEditor.class);
	/**	The HTML Text		*/
	...
	//
	private CTextPane editorPane = new CTextPane();
	//
	...
	private void createMenuBar() 
	{
		...
		for (Action element : actionArray) {
			Object name = element.getValue(Action.NAME);
		//	System.out.println (name);
			actions.put(name, element);
		}
		for (InsertHTMLTextAction element : extraActions) {
			Object name = element.getValue(Action.NAME);
			actions.put(name, element);
		}
	 ...
	 *	@return menu
	 */
	private JMenu buildMenu(String name, HTMLEditor_MenuAction[] menuActions, 
		Hashtable<Object,Action> actions) 
	{
		JMenu menu = new JMenu(name);
		for (HTMLEditor_MenuAction item : menuActions) {
	...
	private void cmd_import()
	{
		...
		jc.setFileFilter(new HtmlFilter());
		//
		...
		{
			log.log(Level.SEVERE, "import" + e.getMessage());
			return;
		}
