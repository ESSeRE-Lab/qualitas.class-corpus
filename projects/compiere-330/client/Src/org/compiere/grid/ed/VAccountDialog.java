	...
	implements ActionListener, DataStatusListener, VetoableChangeListener
{
	...
	//
	private JLabel f_Description = new JLabel ("", SwingConstants.CENTER);
	...
	{
		m_AD_Client_ID = Env.getCtx().getContextAsInt( m_WindowNo, "AD_Client_ID");
		//	Get AcctSchema Info
		...
		Env.getCtx().setContext(m_WindowNo, "C_AcctSchema_ID", m_C_AcctSchema_ID);
		...
		//	Finish
		m_query = new Query();
		m_query.addRestriction("C_AcctSchema_ID", Query.EQUAL, m_C_AcctSchema_ID);
		m_query.addRestriction("IsFullyQualified", Query.EQUAL, "Y");
		if (m_mAccount.C_ValidCombination_ID == 0)
			m_mTab.setQuery(Query.getEqualQuery("1", "2"));
		else
		{
			Query query = new Query();
			query.addRestriction("C_AcctSchema_ID", Query.EQUAL, m_C_AcctSchema_ID);
			query.addRestriction("C_ValidCombination_ID", Query.EQUAL, m_mAccount.C_ValidCombination_ID);
	...
	private void action_Save()
	{
			...
			&& !f_Alias.getValue().toString().equals(Alias))
		{
			...
			int ii = DB.getSQLValue(null, sql1, f_Alias.getValue().toString());
			if (ii != 0)
			{
				ADialog.error(m_WindowNo, this, "DuplicateAlias");
				return;
			}
		...
		else
		{
			...
			{
				String sql1 = "SELECT COUNT(*) FROM C_ValidCombination WHERE Alias=?";
				int ii = DB.getSQLValue(null, sql1, Alias);
				if (ii != 0)
				{
					ADialog.error(m_WindowNo, this, "DuplicateAlias");
					return;
		...
		throws PropertyVetoException
	{
		...
		if (newValue instanceof Integer)
			Env.getCtx().setContext(m_WindowNo, "Account_ID", ((Integer)newValue).intValue());
	}	//	vetoableChange
