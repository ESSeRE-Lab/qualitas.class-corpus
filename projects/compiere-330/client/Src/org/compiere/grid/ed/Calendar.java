	...
	implements ActionListener, MouseListener, ChangeListener, KeyListener
{
	...
	{
		this (frame, Msg.getMsg(Env.getCtx(), "Calendar"), null, DisplayTypeConstants.Date);
	}   //  Calendar
	...
	//	log.config( "Calendar.processWindowEvent", e);
		...
		{
			if (m_displayType == DisplayTypeConstants.Time)
				fHour.requestFocus();
	...
	private void loadData (Timestamp startTS)
	{
		...
		//	What to show
		timePanel.setVisible(m_displayType == DisplayTypeConstants.DateTime || m_displayType == DisplayTypeConstants.Time);
		monthPanel.setVisible(m_displayType != DisplayTypeConstants.Time);
		dayPanel.setVisible(m_displayType != DisplayTypeConstants.Time);
	...
	//	log.config( "Calendar.getTimeStamp");
		...
		else if (m_displayType == DisplayTypeConstants.Time)
