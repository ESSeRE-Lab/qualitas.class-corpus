...
public class MiniTable extends CTable
{
	 ...
	 * Creates a MiniTable.
	 * 
	 * @param withRowHeader true if we have a row header
	 */
	public MiniTable(boolean withRowHeader)
	{
		super(withRowHeader);
	...
	public void addColumn (String header)
	{
		if (getModel() instanceof DefaultTableModel)
		{
			...
			model.addColumn(Util.cleanMnemonic(header));
		}
		else
	 ...
	 *  @param c   class of column - determines renderer
	 *  @param readOnly read only flag
	 */
	public void setColumnClass (int index, Class<?> c, boolean readOnly)
	{
		setColumnClass(index, c, readOnly, null, null);
	}   //  setColumnClass
	...
	{
		setColumnClass(index, c, readOnly, header, null);
	}	//	setColumnClass
	 ...
	 *  @param width optional width
	 */
	public void setColumnClass (int index, Class<?> c, boolean readOnly, String header, Integer width)
	{
	//	log.config( "MiniTable.setColumnClass - " + index, c.getName() + ", r/o=" + readOnly);
		...
		if (header != null && header.length() > 0)
			tc.setHeaderValue(Util.cleanMnemonic(header));
		...
		{
			tc.setCellRenderer(new VCellRenderer(DisplayTypeConstants.Date));
			if (readOnly)
				tc.setCellEditor(new ROCellEditor());
			else
		...
		{
			tc.setCellRenderer(new VCellRenderer(DisplayTypeConstants.Amount));
			if (readOnly)
			{
				tc.setCellEditor(new ROCellEditor());
		...
		{
			tc.setCellRenderer(new VCellRenderer(DisplayTypeConstants.Number));
			if (readOnly)
			{
				tc.setCellEditor(new ROCellEditor());
				m_minWidth.add(Integer.valueOf(70));
		...
		{
			tc.setCellRenderer(new VCellRenderer(DisplayTypeConstants.Integer));
			if (readOnly)
				tc.setCellEditor(new ROCellEditor());
			else
				tc.setCellEditor(new MiniCellEditor(c));
		...
		{
			tc.setCellRenderer(new VCellRenderer(DisplayTypeConstants.String));
			if (readOnly)
				tc.setCellEditor(new ROCellEditor());
			else
			...
			while (rs.next())
			{
				...
				for (int col = 0; col < m_layout.length; col++)
				{
					...
					if (c == IDColumn.class)
						...
						data = Boolean.valueOf(rs.getString(colIndex).equals("Y"));
					else if (c == Timestamp.class)
