...
public class InfoBPartner extends Info
{
	...
	public void saveSelectionDetail()
	{
		...
		Ctx ctx = Env.getCtx();
		ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_BPartner_ID", ID == null ? "0" : ID.toString());
		ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "AD_User_ID", String.valueOf(AD_User_ID));
		ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_BPartner_Location_ID", String.valueOf(C_BPartner_Location_ID));
	}   //  saveSelectionDetail
	...
	//	AEnv.zoom(MBPartner.Table_ID, C_BPartner_ID.intValue(), true);	//	SO
		...
		Query query = new Query("C_BPartner");
		query.addRestriction("C_BPartner_ID", Query.EQUAL, C_BPartner_ID);
