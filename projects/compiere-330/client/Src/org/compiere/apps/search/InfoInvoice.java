...
//	private VLookup fOrg_ID;
	...
	private VNumber fAmtFrom = new VNumber("AmtFrom", false, false, true, DisplayTypeConstants.Amount, Msg.translate(Env.getCtx(), "AmtFrom"));
	private CLabel lAmtTo = new CLabel("-");
	private VNumber fAmtTo = new VNumber("AmtTo", false, false, true, DisplayTypeConstants.Amount, Msg.translate(Env.getCtx(), "AmtTo"));
	...
	//	fPOReference.addActionListener(this);
		...
		fIsSOTrx.setSelected(Env.getCtx().isSOTrx(p_WindowNo));
	...
	//	fOrg_ID.setBackground(CompierePLAF.getInfoBackground());
		...
		fOrder_ID = new VLookup("C_Order_ID", false, false, true,
			MLookupFactory.get (Env.getCtx(), p_WindowNo, 4247, DisplayTypeConstants.Search));
	...
	//		sql.append(" AND UPPER(i.POReference) LIKE ?");
		...
		//sql.append(" AND (i.C_BPartner_ID=? OR i.C_BPartner_ID IN (select c_bpartner_id from C_BP_Relation where c_bpartnerrelation_id=?))");
	...
	void saveSelectionDetail()
	{
		...
		ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_Invoice_ID", ID == null ? "0" : ID.toString());
		//
		...
		if (C_InvoicePaySchedule_ID <= 0)	//	not selected
			ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_InvoicePaySchedule_ID", "0");
		else
			ctx.setContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_InvoicePaySchedule_ID", String.valueOf(C_InvoicePaySchedule_ID));
	}	//	saveSelectionDetail
