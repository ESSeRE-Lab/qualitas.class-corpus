	...
	implements FormPanel, ActionListener, ListSelectionListener
{
	...
	/**	Open Activities				*/
	private ArrayList<MWFActivity> 		m_activities = null;
	/**	Current Activity			*/
	...
	private CTable 		selTable = new CTable(false, selTableModel);
	...
	public int loadActivities()
	{
		...
		String sql = "SELECT * FROM AD_WF_Activity a "
			...
			// Responsible User
			+ " OR EXISTS (SELECT * FROM AD_WF_Responsible r WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID"
			+ " AND r.AD_User_ID=? AND r.responsibletype = 'H')"		//	#3
			//	Responsible Role
			+ " OR EXISTS (SELECT * FROM AD_WF_Responsible r INNER JOIN AD_User_Roles ur ON (r.AD_Role_ID=ur.AD_Role_ID)"
			+ " WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID AND ur.AD_User_ID=? AND r.responsibletype = 'R')"	//	#4
			//
