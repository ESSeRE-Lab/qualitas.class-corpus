...
public class ProcessCtl extends Thread
{
	...
	//	try {System.out.println(">> sleeping ..");sleep(20000);System.out.println(".. sleeping <<");} catch (Exception e) {}
		...
		if (IsReport)
		{
			...
			//	Start Report	-----------------------------------------------
			ReportEngine re = ReportCtl.start( Env.getCtx(), m_pi, IsDirectPrint );
			
			if (m_pi.getAD_Process_ID() == 313 && re == null) // Payment
				ADialog.error(0, null, GlobalMessageConstants.PAYMENT_NOT_COMPLETED);
	...
	{
		if (DB.isDB2())  //jz Only DB2 not support stored procedure now
		{
