	...
	implements FormPanel, ChangeListener, ActionListener
{
	...
	public void init (int WindowNo, FormFrame frame)
	{
		...
		for (MAttribute element : m_attributes)
			vector.add(element.getKeyNamePair());
	...
	private DecimalFormat	m_price = DisplayType.getNumberFormat(DisplayTypeConstants.CostPrice);
	/** Warehouse			*/
	private int				m_M_Warehouse_ID = 0;
	private DecimalFormat	m_qty = DisplayType.getNumberFormat(DisplayTypeConstants.Quantity);
		...
		for (int row = 0; row < rows; row++)
		{
			...
			//	log.fine("Row=" + row + " - Col=" + col);
				...
				if (row == 0 && col == 0)
				{
					...
					if (xValues != null)
						descr.add(new JLabel(m_attributes[indexAttr1-1].getName(),SwingConstants.TRAILING));
					if (yValues != null)
				...
				else if (row == 0)	//	column labels
				{
					if (xValue != null)
					{
						grid.add(new JLabel(xValue.getName(), SwingConstants.TRAILING));
					}
					else
						grid.add(new JLabel());
				}
	...
	private void addProduct(CPanel element, MProduct product)
	{
		...
		pe.add(new JLabel(formatted, SwingConstants.RIGHT), new GridBagConstraints(1,0, 1,1, .5,0, 
		...
		pe.add(new JLabel(formatted, SwingConstants.RIGHT), new GridBagConstraints(1,1, 1,1, .5,0, 
			GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, ii, 0,0));
		//
		element.add(pe);
	}	//	addProduct
	...
	public static void main (String[] args)
	{
		...
		Env.getCtx().setContext("#AD_Client_ID", 11);
