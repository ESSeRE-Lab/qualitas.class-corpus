	...
	implements FormPanel, ActionListener, VetoableChangeListener, ChangeListener, TableModelListener, ASyncProcess
{
	...
	public void init (int WindowNo, FormFrame frame)
	{
		...
		m_WindowNo = WindowNo;
		m_frame = frame;
		Env.getCtx().setContext(m_WindowNo, "IsSOTrx", "Y");
		try
		{
			fillPicks();
			jbInit();
	...
	{
		MLookup orgL = MLookupFactory.get (Env.getCtx(), m_WindowNo, 2163, DisplayTypeConstants.TableDir);
	...
	private void generateInvoice_complete (ProcessInfo pi)
	{
		...
		//	info.append("\n\n" + Msg.getMsg(Env.getCtx(), "PrintInvoices"));
			...
			do
			{
				...
				for (int C_Invoice_ID : ids) {
					ReportCtl.startDocumentPrint( Env.getCtx(), ReportEngine.INVOICE, C_Invoice_ID, true );
				}
