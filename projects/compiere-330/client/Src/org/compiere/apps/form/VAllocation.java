	...
	implements FormPanel, ActionListener, TableModelListener, VetoableChangeListener
{
	...
	public void init (int WindowNo, FormFrame frame)
	{
		m_WindowNo = WindowNo;
		m_frame = frame;
		Env.getCtx().setContext(m_WindowNo, "IsSOTrx", "Y");   //  defaults to no
		m_C_Currency_ID = Env.getCtx().getContextAsInt( "$C_Currency_ID");   //  default
		//
...
//	private int			i_multiplier = 10;
	...
	private void dynInit() throws Exception
	{
		...
		MLookup lookupCur = MLookupFactory.get (Env.getCtx(), m_WindowNo, AD_Column_ID, DisplayTypeConstants.TableDir);
		...
		MLookup lookupBP = MLookupFactory.get (Env.getCtx(), m_WindowNo, AD_Column_ID, DisplayTypeConstants.Search);
		...
		dateField.setValue(new Timestamp(Env.getCtx().getContextAsTime( "#Date")));
...
//				line.add(rs.getBigDecimal(8));		//  6/8-Multiplier
		...
		columnNames.add(Util.cleanMnemonic(Msg.translate(Env.getCtx(), "DocumentNo")));
		if (multiCurrency.isSelected())
		{
...
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  8-Multiplier
		...
		sql = new StringBuffer("SELECT i.DateInvoiced,i.DocumentNo,i.C_Invoice_ID," //  1..3
			...
			*/
			...
			+ "currencyConvert(invoiceDiscount"                               //  8       AllowedDiscount
			+ "(i.C_Invoice_ID," + DB.TO_DATE((Timestamp)dateField.getValue()) + ",C_InvoicePaySchedule_ID),i.C_Currency_ID," + m_C_Currency_ID + ",i.DateInvoiced,i.C_ConversionType_ID,i.AD_Client_ID,i.AD_Org_ID)*i.Multiplier*i.MultiplierAP,"               //  #3, #4
			+ "i.MultiplierAP "
...
//		i_multiplier = multiCurrency.isSelected() ? 10 : 8;
	...
	private void saveData()
	{
		...
		//  fixed fields
		int AD_Client_ID = Env.getCtx().getContextAsInt( m_WindowNo, "AD_Client_ID");
		int AD_Org_ID = Env.getCtx().getContextAsInt( m_WindowNo, "AD_Org_ID");
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				for (int j = 0; j < paymentList.size() && AppliedAmt.signum() != 0; j++)
				{
					...
					if (PaymentAmt.signum() != 0)
					{
						...
						aLine.setAD_Org_ID(pay.getAD_Org_ID());
				...
				if (noPayments == 0 && paymentList.size() == 0)
				{
					...
					MAllocationLine aLine = new MAllocationLine (alloc, AppliedAmt, 
						DiscountAmt, WriteOffAmt, OverUnderAmt);
					aLine.setAD_Org_ID(inv.getAD_Org_ID());
					aLine.setDocInfo(C_BPartner_ID, C_Order_ID, C_Invoice_ID);
					aLine.setPaymentInfo(C_Payment_ID, C_CashLine_ID);
					if (!aLine.save(p_trx))
						log.log(Level.SEVERE, "Allocation Line not written - Invoice=" + C_Invoice_ID);
		...
		for (int i = 0; i < iRows; i++)
		{
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				BigDecimal open = DB.getSQLValueBD(p_trx, sql, C_Invoice_ID);
				if (open != null && open.signum() == 0)
				{
					sql = "UPDATE C_Invoice SET IsPaid='Y' "
						+ "WHERE C_Invoice_ID=" + C_Invoice_ID;
					int no = DB.executeUpdate(sql, p_trx);
					if (no != 0)
		...
		{
			int C_Payment_ID = paymentList.get(i).intValue();
			MPayment pay = new MPayment (Env.getCtx(), C_Payment_ID, p_trx);
