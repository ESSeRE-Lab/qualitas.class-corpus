	...
	implements FormPanel, ActionListener
{
	...
	private VNumber productQty = new VNumber("Qty", true, false, true, DisplayTypeConstants.Quantity, Msg.translate(Env.getCtx(), "Qty"));
	...
	private void createMainPanel ()
	{
		...
		if (m_product != null && m_product.get_ID() > 0)
		{
			...
			m_bomLine = 0;
			addBOMLines(m_product, m_qty, null, X_M_BOM.BOMTYPE_CurrentActive, X_M_BOM.BOMUSE_Master);
		}
	...
	private void addBOMLines (MProduct product, BigDecimal qty, 
							  MBOM bom, String bomType, String bomUse)
	{
		...
		for (MBOMProduct element : bomLines)
			addBOMLine (element, qty, bomType, bomUse);
	 ...
	 * 	@param qty quantity
	 */
	private void addBOMLine (MBOMProduct line, BigDecimal qty, String bomType, String bomUse)
	{
		...
		if (!(componentProduct.isStocked()) && componentProduct.isBOM() && componentProduct.isVerified())
		{
			...
			{
				addBOMLines (componentProduct, lineQty, null, bomType, bomUse);		//	recursive
			}
			else
			{
				MBOM componentBOM = new MBOM(Env.getCtx(), line.getM_ProductBOMVersion_ID(), null);			
				addBOMLines (componentProduct, lineQty, componentBOM, componentBOM.getBOMType(), componentBOM.getBOMUse());
			}
		}
		else
		{
			...
			{
				MBOMAlternative group = new MBOMAlternative(Env.getCtx(), line.getM_BOMAlternative_ID(), null);
				groupName = group.getName();
			}
			addDisplay (line.getBOM().getM_Product_ID(),
					componentProduct.getM_Product_ID(), bomProductType, componentProduct.getName(), lineQty, groupName);
		...
		int M_Product_ID, String bomProductType, String name, BigDecimal lineQty, String groupName)
	{
		...
		else if (X_M_BOMProduct.BOMPRODUCTTYPE_Alternative.equals(bomProductType) ||
				 X_M_BOMProduct.BOMPRODUCTTYPE_AlternativeDefault.equals(bomProductType))	//	Alternative
		{
			String title = groupName;
			JRadioButton b = new JRadioButton(title);
			String radioGroupName = String.valueOf(parentM_Product_ID) + "_" + groupName;
			ButtonGroup group = m_buttonGroups.get(radioGroupName);
			if (group == null)
			{
				log.fine("ButtonGroup=" + radioGroupName);
				group = new ButtonGroup();
				m_buttonGroups.put(radioGroupName, group);
				group.add(b);
		...
		VNumber qty = new VNumber ("Qty", true, false, true, DisplayTypeConstants.Quantity, name);
	...
	private boolean cmd_saveOrder (int C_Order_ID)
	{
		...
		ADialog.info(m_WindowNo, this, "Success", "#" + lineCount);
		return true;
	}	//	cmd_saveOrder
