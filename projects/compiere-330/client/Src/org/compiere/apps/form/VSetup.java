	...
	implements FormPanel, ActionListener, Runnable
{
	...
	private void dynInit()
	{
		...
		//	Country
		sql = "SELECT C_Country_ID, Name FROM C_Country WHERE IsSummary='N' ORDER BY 1";     //  US first
		try
		{
			PreparedStatement pstmt = DB.prepareStatement(sql, (Trx) null);
			ResultSet rs = pstmt.executeQuery();
	...
	private boolean createSetup()
	{
		...
		if (DB.executeUpdate(SQL, (Trx) null) != 0)
		{
...
**/
		...
		if (DB.executeUpdate(SQL, (Trx) null) != 0 || fUserClient.getText().equals(fUserOrg.getText()))
		{
	...
	public void run()
	{
		...
		if (ok)
		{
			...
			int C_Currency_ID = currency.getKey();
			KeyNamePair country = (KeyNamePair)fCountry.getSelectedItem();
			int C_Country_ID = country.getKey();
			if (!ms.createAccounting(C_Country_ID, C_Currency_ID,
			...
			//  Generate Entities
			KeyNamePair region = (KeyNamePair)fRegion.getSelectedItem();
			int C_Region_ID = region.getKey();
			ms.createEntities(fCity.getText(), C_Region_ID);
			info += ms.getInfo();
