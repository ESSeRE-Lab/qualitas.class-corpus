	...
	implements FormPanel, ActionListener, VetoableChangeListener
{
	...
	//	locatorField.addVetoableChangeListener(this);
		...
		//  Dates
		dateFField = new VDate("DateFrom", false, false, true, DisplayTypeConstants.Date, Msg.getMsg(Env.getCtx(), "DateFrom"));
		dateTField = new VDate("DateTo", false, false, true, DisplayTypeConstants.Date, Msg.getMsg(Env.getCtx(), "DateTo"));
		//
	...
	private void dynInit()
	{
		...
		m_staticQuery.addRestriction("AD_Client_ID", Query.EQUAL, Env.getCtx().getAD_Client_ID());
		...
		//
		m_gridController = new GridController(false);
	...
	private void zoom()
	{
		...
		String SQL = null;
		//
		int lineID = Env.getCtx().getContextAsInt( m_WindowNo, "M_InOutLine_ID");
		if (lineID != 0)
		{
			log.fine("M_InOutLine_ID=" + lineID);
			if (Env.getCtx().getContext( m_WindowNo, "MovementType").equals("C-"))
				AD_Window_ID = 169;     //  Customer Shipment
			else if (Env.getCtx().getContext( m_WindowNo, "MovementType").equals("C+"))
				AD_Window_ID = 409;     //  Customer Return
			else if (Env.getCtx().getContext( m_WindowNo, "MovementType").equals("V+"))
		...
		{
			lineID = Env.getCtx().getContextAsInt( m_WindowNo, "M_InventoryLine_ID");
			if (lineID != 0)
			{
			...
			{
				lineID = Env.getCtx().getContextAsInt( m_WindowNo, "M_MovementLine_ID");
				if (lineID != 0)
				{
				...
				{
					lineID = Env.getCtx().getContextAsInt( m_WindowNo, "M_ProductionLine_ID");
					if (lineID != 0)
					{
					...
					{
						lineID = Env.getCtx().getContextAsInt( m_WindowNo, "M_WarehouseTask_ID");
						if (lineID != 0)
						{
							log.fine("M_WarehouseTask_ID=" + lineID);
							AD_Window_ID = ZoomTarget.getZoomAD_Window_ID("M_WarehouseTask", 0, "", true);
							ColumnName = "M_WarehouseTask_ID";
							SQL = "SELECT M_WarehouseTask_ID FROM M_WarehouseTask WHERE M_WarehouseTask_ID=?";
						}
						else
