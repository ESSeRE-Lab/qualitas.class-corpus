	...
	implements FormPanel, ActionListener, TableModelListener, ListSelectionListener
{
	...
	/**	Logger				*/
	static CLogger 			log = CLogger.getCLogger(VMatch.class);
	...
	private VNumber xMatchedTo = new VNumber("xMatchedTo", false, true, false, DisplayTypeConstants.Quantity, "xMatchedTo");
	...
	private CLabel isReturnTrxLabel = new CLabel();
	private JCheckBox isReturnTrx = new JCheckBox();
	...
	private void jbInit() throws Exception
	{
		...
		isReturnTrx.setSelected(false);
		isReturnTrxLabel.setText(Msg.translate(Env.getCtx(), "isReturnTrx"));
		...
		northPanel.add(isReturnTrx,      new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(0, 50, 5, 0), 0, 0));
		northPanel.add(isReturnTrxLabel,        new GridBagConstraints(3, 1, 1, 1, 0.0, 0.0
	...
	//	KeyNamePair BPartner = (KeyNamePair)xMatchedTable.getValueAt(matchedRow, I_BPartner);
		...
		Product.getKey();
		double toMatchQty = m_xMatched.doubleValue();
		double matchedQty = ((Double)xMatchedTable.getValueAt(matchedRow, I_MATCHED)).doubleValue();
	...
	private void tableInit (int display, int matchToType)
	{
		...
		if (display == MATCH_INVOICE)
		{
			...
			m_sql.append("SELECT hdr.C_Invoice_ID,hdr.DocumentNo, hdr.DateInvoiced, bp.Name,hdr.C_BPartner_ID,"
				...
				.append(isReturnTrx.isSelected()? "'Y')":"'N')")
		...
		if (invoice)	//	Shipment - Invoice
		{
			...
			if (iLine.getM_Product_ID() != 0)
			{
				try{
					...
					log.log(Level.SEVERE, "Inv Match not created: " + e.getMessage());
				}
			}
			else
				success = true;
			...
			if (iLine.getC_OrderLine_ID() != 0 && iLine.getM_Product_ID() != 0)
			{
				try{
					...
					log.log(Level.SEVERE, "PO(Inv) Match not created: " + e.getMessage());					
				}
			}
		}
		else	//	Shipment - Order
		{
			...
			if (sLine.getM_Product_ID() != 0)
			{
				...
				else
				{
					success = true;
					int M_Locator_ID = 0;
					//	Get Locator to reverse reservation
					...
					if (sLine.getProduct() != null && sLine.getProduct().isStocked())
						success = MStorage.add (Env.getCtx(), oLine.getM_Warehouse_ID(), 
							M_Locator_ID,	
