	...
	implements ActionListener, VetoableChangeListener
{
	...
	//
	private final ArrayList<VEditor>	m_vEditors = new ArrayList<VEditor>();
	private final ArrayList<VEditor>	m_vEditors2 = new ArrayList<VEditor>();		//	for ranges
	private final ArrayList<GridField>	m_mFields = new ArrayList<GridField>();
	private final ArrayList<GridField>	m_mFields2 = new ArrayList<GridField>();
	//
    ...
    private final MultiMap<String,GridField>      m_depOnField = new MultiMap<String,GridField>();
	...
	private void createField (ResultSet rs)
	{
        ...
        // List of fields that this field is dependent on
		GridField field = mField;
		ArrayList<String> list = field.getDependentOn(false);
	...
	{
		// the setting of default values cannot be done in the createField()
		// loop since setting field values changes the context and then affects
		// the resolving of subsequent field validations

		for( GridField mField : m_mFields )
		{
			if( mField != null )
			{
				//  Set Default
				Object defaultObject = mField.getDefault(Env.getCtx(), m_WindowNo);
				mField.setValue (defaultObject, true);
			}
		}

		for( GridField mField2 : m_mFields2 )
		{
			if( mField2 != null )
			{
				//  Set Default
				Object defaultObject2 = mField2.getDefault(Env.getCtx(), m_WindowNo);
	...
	public void getModalCtl(ASyncProcess parent,ProcessInfo pi, Trx p_trx)
	{
		m_parent=parent;
		m_processInfo=pi;
		m_trx=p_trx;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getActionCommand().equals(ConfirmPanel.A_OK))
		{
			...
			if (saveParameters())
			{
				m_isOK = true;
				dispose();
				ProcessCtl worker = new ProcessCtl(m_parent ,m_processInfo,m_trx);
				worker.start();
			}
		}
	...
	//	log.fine( "ProcessParameter.vetoableChange");
        ...
        String columnName = evt.getPropertyName();
		Env.getCtx().setContext(m_WindowNo, columnName, value);

		if (m_depOnField.containsKey(columnName)) {
			ArrayList<GridField> dependentFields = m_depOnField.getValues(columnName);
			for (GridField field : dependentFields)
			{
				GridField dep = field;
				if (dep == null)
					continue;
				dep.refreshLookup();
				dep.setValue(dep.getDefault(Env.getCtx(), m_WindowNo), true);
			}
		}
	}	// vetoableChange
	...
	private boolean saveParameters()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
				VEditor vEditor = m_vEditors.get(i);
				...
				//  Check for Range
				VEditor vEditor2 = m_vEditors2.get(i);
				if (vEditor2 != null)
				{
		...
		for (int i = 0; i < m_mFields.size(); i++)
		{
			...
			VEditor editor2 = m_vEditors2.get(i);
