...
public class RequestServlet extends HttpServlet
{
	...
	//	int AD_Client_ID = ctx.getContextAsInt( "AD_Client_ID");
		...
		if (R_Request_ID == 0)
		{
			...
			if (IsConfidential)
				req.setConfidentialType(X_R_Request.CONFIDENTIALTYPE_PartnerConfidential);
			//
		...
		else	//	existing Request
		{
			...
			//	Flags
			if (IsConfidential)
				req.setConfidentialTypeEntry(X_R_Request.CONFIDENTIALTYPEENTRY_PartnerConfidential);
			if (IsClose)
				req.doClose();
			else if (IsEscalate)
			{
		...
		JSPEnv.sendEMail(request, wu, X_W_MailMsg.MAILMSGTYPE_Request, 
			new Object[]{
				req.getDocumentNo(),
				wu.getName(),
				message,
