...
public class JSPEnv
{
	 ...
	 * 	@return properties
	 */
	protected static Ctx getCtx (HttpServletRequest request)
	{
		WebSessionCtx wsc = WebSessionCtx.get(request, true);	//	create new
		...
		if (wu != null)
		{
			...
			wsc.ctx.setAD_User_ID(AD_User_ID);		//	security
		}

		//	Finish
		session.setMaxInactiveInterval(1800);	//	30 Min	HARDCODED
		String info = wsc.ctx.get(WebSessionCtx.HDR_INFO);
		if (info != null)
	...
	public static String getCookieWebUser (HttpServletRequest request)
	{
		...
		for (Cookie element : cookies) {
			if (COOKIE_NAME.equals(element.getName()))
