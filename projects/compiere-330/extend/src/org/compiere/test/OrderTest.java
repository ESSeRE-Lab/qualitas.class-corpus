...
public class OrderTest implements Runnable
{
	...
	public void run()
	{
		...
		bp.setSOCreditStatus(X_C_BPartner.SOCREDITSTATUS_NoCreditCheck);
		bp.save();
		
		//
		for (int i = 0; i < m_numberOrders; i++)
		{
			Trx p_trx = Trx.get("Test" + m_no + "_" + i);
			//
			MOrder order = new MOrder(Env.getCtx(),0,p_trx);
			...
			//	Process
			order.setDocAction(DocActionConstants.ACTION_Complete);
			if (!order.processIt(DocActionConstants.ACTION_Complete))
			{
